...
static int get_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_num_stages(
        int block_n, int block_m);

class SM90HybridSparseGroupStageOutput64x64NM12FastpathDescReuseFixedShapeRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupStageOutput64x64NM12FastpathDescReuseFixedShapeRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape.cuh>
...
static void __instantiate_kernel() {{
    ...
    (void)ptr;
            ...
            get_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_num_stages(
                args.block_n, args.block_m),
...
static void sm90_hybrid_block_sparse_bf16_gemm_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape(
    ...
    const int num_stages =
        get_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_num_stages(block_n, block_m);
    const int stage_bytes =
        dense_count * 64 * 64 * sizeof(__nv_bfloat16) +
        block_n * 64 * 32 * sizeof(__nv_bfloat16) +
        64 * 64 * block_m * sizeof(__nv_bfloat16) +
    ...
    const auto args = SM90HybridSparseGroupStageOutput64x64NM12FastpathDescReuseFixedShapeRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape",
        SM90HybridSparseGroupStageOutput64x64NM12FastpathDescReuseFixedShapeRuntime::generate(args));
    SM90HybridSparseGroupStageOutput64x64NM12FastpathDescReuseFixedShapeRuntime::launch(runtime, args);
}
