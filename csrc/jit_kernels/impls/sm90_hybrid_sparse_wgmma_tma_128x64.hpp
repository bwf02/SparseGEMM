...
enum class HybridSparseWgmmaTma128x64Kernel { Dense, Sparse, Reduce };

class SM90HybridSparseWgmmaTma128x64Runtime final:
        public LaunchRuntime<SM90HybridSparseWgmmaTma128x64Runtime> {
public:
    struct Args {
        HybridSparseWgmmaTma128x64Kernel kernel;
...
void hybrid_sparse_dense_wgmma_tma_128x64(
    ...
    constexpr int kWeightBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kActivationBytes = kTileM * kBlock * sizeof(__nv_bfloat16);
...
#pragma unroll
                for (int k_tile = 0; k_tile < 4; ++k_tile) {
                    ...
                    const auto desc_b = deep_gemm::mma::sm90::make_smem_desc(
                        smem_activation(stage) + m_tile * kBlock * kBlock +
                            k_tile * 16,
                        static_cast<int>(cute::GMMA::LayoutType::B128), 0, 1024);
                    DenseMMA::wgmma(desc_a.desc_, desc_b.desc_,
                                    accumulator + m_tile * 32,
...
#pragma unroll
    for (int m_tile = 0; m_tile < 2; ++m_tile)
        store_wgmma_accumulator(accumulator + m_tile * 32, dense_partial,
                                output_tile_m + m_tile * kBlock,
                                output_tile_n, m, n);
}
)";
...
extern "C" __global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_2_4_wgmma_tma_128x64(
        ...
        for (int local_block = 0; local_block < block_m; ++local_block) {
            ...
            const long long metadata_base =
                (((static_cast<long long>(block_row) * block_groups + block_group)
                   * block_n + sparse_slot) * kBlock) * (kBlock / 4);
            full_barrier[stage].wait(phase);
#pragma unroll
            for (int i = 0; i < 64; ++i)
                deep_gemm::ptx::warpgroup_fence_operand(accumulator[i]);
...
#pragma unroll
    for (int m_tile = 0; m_tile < 2; ++m_tile)
        store_wgmma_accumulator(accumulator + m_tile * 32, sparse_partial,
                                output_tile_m + m_tile * kBlock,
                                output_tile_n, m, n);
}
)";
...
extern "C" __global__ void hybrid_sparse_reduce_wgmma_tma_128x64(
    ...
    for (int linear = static_cast<int>(threadIdx.x); linear < kTileM * kBlock;
        ...
        switch (args.kernel) {
            case HybridSparseWgmmaTma128x64Kernel::Dense:
                return std::string(common) + dense;
            case HybridSparseWgmmaTma128x64Kernel::Sparse:
                return std::string(common) + sparse;
            case HybridSparseWgmmaTma128x64Kernel::Reduce:
                return std::string(common) + reduce;
        }
        DG_HOST_UNREACHABLE("Unknown hybrid sparse WGMMA TMA 128x64 kernel");
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_128x64(
    ...
    constexpr int dense_smem_bytes =
        2 * (64 * 64 + 128 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    constexpr int sparse_smem_bytes =
        2 * (64 * 32 + 128 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const auto tensor_map_sparse = make_tma_2d_desc(
        ...
        32, 64, 32, 64);
    const auto grid = std::make_pair((m + 127) / 128, (n + 63) / 64);
    auto args = SM90HybridSparseWgmmaTma128x64Runtime::Args {
        .kernel = HybridSparseWgmmaTma128x64Kernel::Dense,
    ...
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma_128x64",
        SM90HybridSparseWgmmaTma128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTma128x64Runtime::launch(dense_runtime, args);
    args.kernel = HybridSparseWgmmaTma128x64Kernel::Sparse;
    args.launch_args = LaunchArgs(grid, 256, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma_128x64",
        SM90HybridSparseWgmmaTma128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTma128x64Runtime::launch(sparse_runtime, args);
    args.kernel = HybridSparseWgmmaTma128x64Kernel::Reduce;
    args.launch_args = LaunchArgs(grid, 256);
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma_128x64",
        SM90HybridSparseWgmmaTma128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTma128x64Runtime::launch(reduce_runtime, args);
}
