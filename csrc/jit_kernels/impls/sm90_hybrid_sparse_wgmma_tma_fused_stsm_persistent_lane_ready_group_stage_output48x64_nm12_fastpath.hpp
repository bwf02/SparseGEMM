...
static int get_group_stage_output48x64_nm12_fastpath_num_stages(
        int block_n, int block_m);

class SM90HybridSparseGroupStageOutput48x64NM12FastpathRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupStageOutput48x64NM12FastpathRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output48x64_nm12_fastpath.cuh>

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_group_stage_output48x64_nm12_fastpath<{}, {}, {}>);
    (void)ptr;
}}
)",
            args.block_n, args.block_m,
            get_group_stage_output48x64_nm12_fastpath_num_stages(
                args.block_n, args.block_m));
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_group_stage_output48x64_nm12_fastpath(
    ...
    const int num_stages =
        get_group_stage_output48x64_nm12_fastpath_num_stages(block_n, block_m);
    const int stage_bytes =
        dense_count * 64 * 64 * sizeof(__nv_bfloat16) +
        block_n * 64 * 32 * sizeof(__nv_bfloat16) +
        48 * 64 * block_m * sizeof(__nv_bfloat16) +
    ...
    const auto args = SM90HybridSparseGroupStageOutput48x64NM12FastpathRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_group_stage_output48x64_nm12_fastpath",
        SM90HybridSparseGroupStageOutput48x64NM12FastpathRuntime::generate(args));
    SM90HybridSparseGroupStageOutput48x64NM12FastpathRuntime::launch(runtime, args);
}
