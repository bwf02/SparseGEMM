...
class SM90HybridSparseOutput128x64StageKindDescReuseCompactRuntime final:
        public LaunchRuntime<SM90HybridSparseOutput128x64StageKindDescReuseCompactRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output128x64_stage_kind_desc_reuse_compact.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output128x64_stage_kind_desc_reuse_compact<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output128x64_stage_kind_desc_reuse_compact(
    ...
    constexpr int stage_bytes =
        (64 * 64 + 128 * 64) * sizeof(__nv_bfloat16) +
    ...
    const auto args = SM90HybridSparseOutput128x64StageKindDescReuseCompactRuntime::Args {
        ...
        .launch_args = LaunchArgs(persistent_ctas, 320, smem_bytes),
    };
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output128x64_stage_kind_desc_reuse_compact",
        SM90HybridSparseOutput128x64StageKindDescReuseCompactRuntime::generate(args));
    SM90HybridSparseOutput128x64StageKindDescReuseCompactRuntime::launch(runtime, args);
}
