...
class SM90HybridSparseFusedStsmPersistentLaneReadyStage3Runtime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyStage3Runtime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_stage3.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage3<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_stage3(
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyStage3Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage3",
        SM90HybridSparseFusedStsmPersistentLaneReadyStage3Runtime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyStage3Runtime::launch(
        runtime, args);
}
