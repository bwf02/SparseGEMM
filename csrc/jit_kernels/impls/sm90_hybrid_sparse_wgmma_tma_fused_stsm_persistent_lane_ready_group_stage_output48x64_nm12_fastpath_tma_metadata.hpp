...
static int get_group_stage_output48x64_nm12_fastpath_tma_metadata_num_stages(
        int block_n, int block_m);

class SM90HybridSparseGroupStageOutput48x64NM12FastpathTmaMetadataRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupStageOutput48x64NM12FastpathTmaMetadataRuntime> {
public:
    struct Args {
        void *block_selector, *hardware_metadata, *d;
        CUtensorMap tensor_map_activation;
        CUtensorMap tensor_map_dense;
        CUtensorMap tensor_map_sparse;
        CUtensorMap tensor_map_metadata;
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output48x64_nm12_fastpath_tma_metadata.cuh>

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_group_stage_output48x64_nm12_fastpath_tma_metadata<{}, {}, {}>);
    (void)ptr;
}}
)",
            args.block_n, args.block_m,
            get_group_stage_output48x64_nm12_fastpath_tma_metadata_num_stages(
                args.block_n, args.block_m));
    }
    ...
    static void launch_impl(const KernelHandle& kernel,
        ...
        DG_CUDA_UNIFIED_CHECK(launch_kernel(
            ...
            args.tensor_map_sparse, args.tensor_map_metadata,
...
static int get_group_stage_output48x64_nm12_fastpath_tma_metadata_num_stages(
    ...
    constexpr int output_bytes = 48 * 64 * sizeof(__nv_bfloat16);
    ...
    const int stage_bytes =
        ...
        48 * 64 * block_m * sizeof(__nv_bfloat16) +
...
static void sm90_hybrid_block_sparse_bf16_gemm_group_stage_output48x64_nm12_fastpath_tma_metadata(
    ...
    const int num_stages =
        get_group_stage_output48x64_nm12_fastpath_tma_metadata_num_stages(block_n, block_m);
    const int stage_bytes =
        dense_count * 64 * 64 * sizeof(__nv_bfloat16) +
        block_n * 64 * 32 * sizeof(__nv_bfloat16) +
        48 * 64 * block_m * sizeof(__nv_bfloat16) +
    ...
    const auto tensor_map_sparse = make_tma_2d_desc(
        ...
        32, 64, 32, 64);
    const auto tensor_map_metadata = make_tma_2d_desc(
        hardware_metadata, 32,
        block_rows * block_groups * block_n * 4,
        32, 4, 32, 0);
    const auto tensor_map_output = make_tma_cd_desc(
        d, m, n, 48, 64, n, 1, 128);
    const int total_tiles = ((m + 47) / 48) * ((n + 63) / 64);
    ...
    const auto args = SM90HybridSparseGroupStageOutput48x64NM12FastpathTmaMetadataRuntime::Args {
        ...
        .tensor_map_metadata = tensor_map_metadata,
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_group_stage_output48x64_nm12_fastpath_tma_metadata",
        SM90HybridSparseGroupStageOutput48x64NM12FastpathTmaMetadataRuntime::generate(args));
    SM90HybridSparseGroupStageOutput48x64NM12FastpathTmaMetadataRuntime::launch(runtime, args);
}
