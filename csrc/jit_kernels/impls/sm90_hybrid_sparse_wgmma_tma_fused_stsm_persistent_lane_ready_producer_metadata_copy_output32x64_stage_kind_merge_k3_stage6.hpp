...
class SM90HybridSparseOutput32x64StageKindMergeK3Stage6Runtime final:
        public LaunchRuntime<SM90HybridSparseOutput32x64StageKindMergeK3Stage6Runtime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output32x64_stage_kind_merge_k3_stage6.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output32x64_stage_kind_merge_k3_stage6<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output32x64_stage_kind_merge_k3_stage6(
    ...
    constexpr int barrier_bytes = 12 * sizeof(std::uint64_t);
    constexpr int stage_bytes =
        (64 * 64 + 32 * 64) * sizeof(__nv_bfloat16) +
        2 * 4 * 16 * sizeof(std::uint32_t);
    constexpr int pipeline_bytes = 6 * stage_bytes + barrier_bytes;
    ...
    const auto args = SM90HybridSparseOutput32x64StageKindMergeK3Stage6Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output32x64_stage_kind_merge_k3_stage6",
        SM90HybridSparseOutput32x64StageKindMergeK3Stage6Runtime::generate(args));
    SM90HybridSparseOutput32x64StageKindMergeK3Stage6Runtime::launch(runtime, args);
}
