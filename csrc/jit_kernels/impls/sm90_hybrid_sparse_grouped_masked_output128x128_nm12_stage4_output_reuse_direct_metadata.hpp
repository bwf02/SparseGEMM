...
class SM90HybridSparseGroupedMaskedOutput128x128NM12Stage4OutputReuseDirectMetadataRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupedMaskedOutput128x128NM12Stage4OutputReuseDirectMetadataRuntime> {
public:
    struct Args {
        void* block_selector;
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_grouped_masked_output128x128_nm12_stage4_output_reuse_direct_metadata.cuh>

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_grouped_masked_output128x128_nm12_stage4_output_reuse_direct_metadata<
            {}, {}, {}, {}, {}, {}, {}>);
    (void)ptr;
}}
)",
            args.block_n, args.block_m, 4, args.num_experts,
...
static void sm90_hybrid_block_sparse_bf16_grouped_masked_output128x128_nm12_stage4_output_reuse_direct_metadata(
    ...
    const int stage_bytes =
        2 * 64 * 64 * sizeof(__nv_bfloat16) +
        2 * 64 * 32 * sizeof(__nv_bfloat16) +
    ...
    const int total_tiles =
        num_experts * (max_m / 128) * (n / 128);
    const auto args =
        SM90HybridSparseGroupedMaskedOutput128x128NM12Stage4OutputReuseDirectMetadataRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_grouped_masked_output128x128_nm12_stage4_output_reuse_direct_metadata",
        SM90HybridSparseGroupedMaskedOutput128x128NM12Stage4OutputReuseDirectMetadataRuntime::generate(args));
    SM90HybridSparseGroupedMaskedOutput128x128NM12Stage4OutputReuseDirectMetadataRuntime::launch(runtime, args);
}
