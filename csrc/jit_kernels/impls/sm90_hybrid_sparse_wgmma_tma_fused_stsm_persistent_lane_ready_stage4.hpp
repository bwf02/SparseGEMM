...
class SM90HybridSparseFusedStsmPersistentLaneReadyStage4Runtime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyStage4Runtime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_stage4.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage4<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_stage4(
    ...
    constexpr int barrier_bytes = 8 * sizeof(std::uint64_t);
    constexpr int pipeline_bytes =
        4 * (64 * 64 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyStage4Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage4",
        SM90HybridSparseFusedStsmPersistentLaneReadyStage4Runtime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyStage4Runtime::launch(
        runtime, args);
}
