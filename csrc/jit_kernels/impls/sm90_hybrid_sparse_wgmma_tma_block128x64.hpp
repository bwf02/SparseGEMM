...
enum class HybridSparseWgmmaTmaBlock128x64Kernel { Dense, Sparse, Reduce };

class SM90HybridSparseWgmmaTmaBlock128x64Runtime final:
        public LaunchRuntime<SM90HybridSparseWgmmaTmaBlock128x64Runtime> {
public:
    struct Args {
        HybridSparseWgmmaTmaBlock128x64Kernel kernel;
    ...
    static const char* kernel_symbol(
            const HybridSparseWgmmaTmaBlock128x64Kernel kernel) {
        switch (kernel) {
            case HybridSparseWgmmaTmaBlock128x64Kernel::Dense:
                return "hybrid_sparse_dense_wgmma_tma_block128x64";
            case HybridSparseWgmmaTmaBlock128x64Kernel::Sparse:
                return "hybrid_sparse_2_4_wgmma_tma_block128x64";
            case HybridSparseWgmmaTmaBlock128x64Kernel::Reduce:
                return "hybrid_sparse_reduce_wgmma_tma_block128x64";
        }
    ...
    static std::string generate_impl(const Args& args) {
        ...
        return std::string(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_block128x64.cuh>
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_block128x64(
    ...
    constexpr int dense_smem_bytes =
        2 * (128 * 64 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    constexpr int sparse_smem_bytes =
        2 * (128 * 32 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const auto tensor_map_dense = dense_count > 0 ? make_tma_2d_desc(
        dense_values, 64, block_rows * block_groups * dense_count * 128,
        64, 128, 64, 128) : tensor_map_activation;
    const auto tensor_map_sparse = make_tma_2d_desc(
        sparse_values, 32, block_rows * block_groups * block_n * 128,
        32, 128, 32, 64);
    const auto grid = std::make_pair((m + 63) / 64, (n + 127) / 128);
    auto args = SM90HybridSparseWgmmaTmaBlock128x64Runtime::Args {
        .kernel = HybridSparseWgmmaTmaBlock128x64Kernel::Dense,
    ...
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma_block128x64",
        SM90HybridSparseWgmmaTmaBlock128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x64Runtime::launch(dense_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x64Kernel::Sparse;
    args.launch_args = LaunchArgs(grid, 256, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma_block128x64",
        SM90HybridSparseWgmmaTmaBlock128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x64Runtime::launch(sparse_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x64Kernel::Reduce;
    args.launch_args = LaunchArgs(grid, 256);
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma_block128x64",
        SM90HybridSparseWgmmaTmaBlock128x64Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x64Runtime::launch(reduce_runtime, args);
}
