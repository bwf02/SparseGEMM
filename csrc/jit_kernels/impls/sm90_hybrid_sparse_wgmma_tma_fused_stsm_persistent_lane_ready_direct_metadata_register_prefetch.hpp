...
class SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataRegisterPrefetchRuntime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataRegisterPrefetchRuntime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_register_prefetch.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_register_prefetch<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_register_prefetch(
    ...
    constexpr int pipeline_bytes =
        3 * (64 * 64 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataRegisterPrefetchRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_register_prefetch",
        SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataRegisterPrefetchRuntime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataRegisterPrefetchRuntime::launch(
        runtime, args);
}
