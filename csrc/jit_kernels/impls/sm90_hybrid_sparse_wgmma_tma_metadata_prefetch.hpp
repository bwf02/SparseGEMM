...
enum class HybridSparseMetadataPrefetchKernel { Dense, Sparse, Reduce };

class SM90HybridSparseMetadataPrefetchRuntime final:
        public LaunchRuntime<SM90HybridSparseMetadataPrefetchRuntime> {
public:
    struct Args {
        HybridSparseMetadataPrefetchKernel kernel;
    ...
    static const char* kernel_symbol(
            const HybridSparseMetadataPrefetchKernel kernel) {
        switch (kernel) {
            case HybridSparseMetadataPrefetchKernel::Dense:
                return "hybrid_sparse_dense_wgmma_tma";
            case HybridSparseMetadataPrefetchKernel::Sparse:
                return "hybrid_sparse_2_4_wgmma_tma_metadata_prefetch";
            case HybridSparseMetadataPrefetchKernel::Reduce:
                return "hybrid_sparse_reduce_wgmma_tma";
        }
        DG_HOST_UNREACHABLE("Unknown metadata prefetch kernel");
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_metadata_prefetch(
    ...
    constexpr int sparse_pipeline_bytes =
        2 * (64 * 32 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const int sparse_smem_bytes = sparse_pipeline_bytes + metadata_bytes;
    ...
    auto args = SM90HybridSparseMetadataPrefetchRuntime::Args {
        .kernel = HybridSparseMetadataPrefetchKernel::Dense,
    ...
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma_metadata_prefetch",
        SM90HybridSparseMetadataPrefetchRuntime::generate(args));
    SM90HybridSparseMetadataPrefetchRuntime::launch(dense_runtime, args);
    args.kernel = HybridSparseMetadataPrefetchKernel::Sparse;
    args.launch_args = LaunchArgs(grid, 256, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma_metadata_prefetch",
        SM90HybridSparseMetadataPrefetchRuntime::generate(args));
    SM90HybridSparseMetadataPrefetchRuntime::launch(sparse_runtime, args);
    args.kernel = HybridSparseMetadataPrefetchKernel::Reduce;
    args.launch_args = LaunchArgs(grid, 256);
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma_metadata_prefetch",
        SM90HybridSparseMetadataPrefetchRuntime::generate(args));
    SM90HybridSparseMetadataPrefetchRuntime::launch(reduce_runtime, args);
}
