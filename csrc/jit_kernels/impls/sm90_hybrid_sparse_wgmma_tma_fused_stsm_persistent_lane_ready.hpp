...
class SM90HybridSparseFusedStsmPersistentLaneReadyRuntime final:
        public LaunchRuntime<SM90HybridSparseFusedStsmPersistentLaneReadyRuntime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready(
    ...
    const auto args = SM90HybridSparseFusedStsmPersistentLaneReadyRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready",
        SM90HybridSparseFusedStsmPersistentLaneReadyRuntime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyRuntime::launch(runtime, args);
}
