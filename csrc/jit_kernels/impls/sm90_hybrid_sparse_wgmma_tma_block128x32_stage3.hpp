...
enum class HybridSparseWgmmaTmaBlock128x32Stage3Kernel {
    Dense, Sparse, Reduce
};

class SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime final:
        public LaunchRuntime<SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime> {
public:
    struct Args {
        HybridSparseWgmmaTmaBlock128x32Stage3Kernel kernel;
    ...
    static const char* kernel_symbol(
            const HybridSparseWgmmaTmaBlock128x32Stage3Kernel kernel) {
        switch (kernel) {
            case HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Dense:
                return "hybrid_sparse_dense_wgmma_tma_block128x32_stage3";
            case HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Sparse:
                return "hybrid_sparse_2_4_wgmma_tma_block128x32_stage3";
            case HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Reduce:
                return "hybrid_sparse_reduce_wgmma_tma_block128x32_stage3";
        }
    ...
    static std::string generate_impl(const Args& args) {
        ...
        return std::string(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_block128x32_stage3.cuh>
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_block128x32_stage3(
    ...
    constexpr int dense_smem_bytes =
        3 * (128 * 32 + 64 * 32) * sizeof(__nv_bfloat16) + barrier_bytes;
    constexpr int sparse_smem_bytes =
        3 * (128 * 16 + 64 * 32) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    auto args = SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::Args {
        .kernel = HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Dense,
    ...
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma_block128x32_stage3",
        SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::launch(dense_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Sparse;
    args.launch_args = LaunchArgs(grid, 256, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma_block128x32_stage3",
        SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::launch(sparse_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x32Stage3Kernel::Reduce;
    args.launch_args = LaunchArgs(grid, 256);
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma_block128x32_stage3",
        SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Stage3Runtime::launch(reduce_runtime, args);
}
