...
class SM90HybridSparseFusedStsmPersistentLaneReadyTmaMetadataRuntime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyTmaMetadataRuntime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_tma_metadata.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_tma_metadata<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_tma_metadata(
    ...
    constexpr int pipeline_bytes =
        3 * ((64 * 64 + 64 * 64) * sizeof(__nv_bfloat16) +
             2 * 4 * 16 * sizeof(std::uint32_t)) + barrier_bytes;
    constexpr int output_offset = ((pipeline_bytes + 1023) / 1024) * 1024;
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyTmaMetadataRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_tma_metadata",
        SM90HybridSparseFusedStsmPersistentLaneReadyTmaMetadataRuntime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyTmaMetadataRuntime::launch(
        runtime, args);
}
