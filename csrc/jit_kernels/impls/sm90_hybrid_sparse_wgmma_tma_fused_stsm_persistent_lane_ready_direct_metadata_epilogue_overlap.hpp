...
class SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataEpilogueOverlapRuntime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataEpilogueOverlapRuntime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap(
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataEpilogueOverlapRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap",
        SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataEpilogueOverlapRuntime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyDirectMetadataEpilogueOverlapRuntime::launch(
        runtime, args);
}
