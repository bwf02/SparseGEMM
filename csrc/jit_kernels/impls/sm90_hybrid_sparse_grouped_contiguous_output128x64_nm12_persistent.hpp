...
class SM90HybridSparseGroupedContiguousOutput128x64NM12PersistentRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupedContiguousOutput128x64NM12PersistentRuntime> {
public:
    struct Args {
        void* block_selector;
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_grouped_contiguous_output128x64_nm12_persistent<
            {}, {}, {}, {}, {}, {}, {}>);
    (void)ptr;
}}
)",
...
static void sm90_hybrid_block_sparse_bf16_grouped_contiguous_output128x64_nm12_persistent(
    ...
    const auto tensor_map_output = make_tma_cd_desc(
        d, total_m, n, 128, 64, n, 1, 128);
    const int total_tiles =
        (total_m / 128) * ((n + 63) / 64);
    const auto args =
        SM90HybridSparseGroupedContiguousOutput128x64NM12PersistentRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_grouped_contiguous_output128x64_nm12_persistent",
        SM90HybridSparseGroupedContiguousOutput128x64NM12PersistentRuntime::generate(args));
    SM90HybridSparseGroupedContiguousOutput128x64NM12PersistentRuntime::launch(runtime, args);
}
