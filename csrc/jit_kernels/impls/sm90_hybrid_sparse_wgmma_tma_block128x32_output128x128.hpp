...
enum class HybridSparseWgmmaTmaBlock128x32Output128x128Kernel { Dense, Sparse, Reduce };

class SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime final:
        public LaunchRuntime<SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime> {
public:
    struct Args {
        HybridSparseWgmmaTmaBlock128x32Output128x128Kernel kernel;
    ...
    static const char* kernel_symbol(
            const HybridSparseWgmmaTmaBlock128x32Output128x128Kernel kernel) {
        switch (kernel) {
            case HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Dense:
                return "hybrid_sparse_dense_wgmma_tma_block128x32_output128x128";
            case HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Sparse:
                return "hybrid_sparse_2_4_wgmma_tma_block128x32_output128x128";
            case HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Reduce:
                return "hybrid_sparse_reduce_wgmma_tma_block128x32_output128x128";
        }
    ...
    static std::string generate_impl(const Args& args) {
        ...
        return std::string(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_block128x32_output128x128.cuh>
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_block128x32_output128x128(
    ...
    constexpr int dense_smem_bytes =
        2 * (128 * 32 + 128 * 32) * sizeof(__nv_bfloat16) + barrier_bytes;
    constexpr int sparse_smem_bytes =
        2 * (128 * 16 + 128 * 32) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const auto tensor_map_sparse = make_tma_2d_desc(
        ...
        16, 128, 16, 32);
    const auto grid = std::make_pair((m + 127) / 128, (n + 127) / 128);
    auto args = SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::Args {
        .kernel = HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Dense,
        ...
        .launch_args = LaunchArgs(grid, 384, dense_smem_bytes),
    };
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma_block128x32_output128x128",
        SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::launch(dense_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Sparse;
    args.launch_args = LaunchArgs(grid, 384, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma_block128x32_output128x128",
        SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::launch(sparse_runtime, args);
    args.kernel = HybridSparseWgmmaTmaBlock128x32Output128x128Kernel::Reduce;
    ...
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma_block128x32_output128x128",
        SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::generate(args));
    SM90HybridSparseWgmmaTmaBlock128x32Output128x128Runtime::launch(reduce_runtime, args);
}
