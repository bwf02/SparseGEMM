...
class SM90HybridSparseProducerMetadataCopyOutput128x64Runtime final:
        public LaunchRuntime<
            SM90HybridSparseProducerMetadataCopyOutput128x64Runtime> {
public:
    struct Args {
...
static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_producer_metadata_copy_output128x64<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_producer_metadata_copy_output128x64(
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_producer_metadata_copy_output128x64",
        SM90HybridSparseProducerMetadataCopyOutput128x64Runtime::generate(args));
    SM90HybridSparseProducerMetadataCopyOutput128x64Runtime::launch(
        runtime, args);
}
