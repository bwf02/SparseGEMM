...
class SM90HybridSparseOutput64x64StageKindRuntime final:
        public LaunchRuntime<SM90HybridSparseOutput64x64StageKindRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output64x64_stage_kind.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output64x64_stage_kind<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output64x64_stage_kind(
    ...
    const auto args = SM90HybridSparseOutput64x64StageKindRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output64x64_stage_kind",
        SM90HybridSparseOutput64x64StageKindRuntime::generate(args));
    SM90HybridSparseOutput64x64StageKindRuntime::launch(runtime, args);
}
