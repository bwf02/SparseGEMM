...
class SM90HybridSparseOutput128x64StageSelectorRuntime final:
        public LaunchRuntime<SM90HybridSparseOutput128x64StageSelectorRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output128x64_stage_selector.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output128x64_stage_selector<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output128x64_stage_selector(
    ...
    const auto args = SM90HybridSparseOutput128x64StageSelectorRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output128x64_stage_selector",
        SM90HybridSparseOutput128x64StageSelectorRuntime::generate(args));
    SM90HybridSparseOutput128x64StageSelectorRuntime::launch(runtime, args);
}
