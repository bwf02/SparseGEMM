...
enum class HybridSparseWgmmaTmaKernel { Dense, Sparse, Reduce };

class SM90HybridSparseWgmmaTmaRuntime final:
        public LaunchRuntime<SM90HybridSparseWgmmaTmaRuntime> {
public:
    struct Args {
        HybridSparseWgmmaTmaKernel kernel;
    ...
    static const char* kernel_symbol(const HybridSparseWgmmaTmaKernel kernel) {
        switch (kernel) {
            case HybridSparseWgmmaTmaKernel::Dense:
                return "hybrid_sparse_dense_wgmma_tma";
            case HybridSparseWgmmaTmaKernel::Sparse:
                return "hybrid_sparse_2_4_wgmma_tma";
            case HybridSparseWgmmaTmaKernel::Reduce:
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma(
    ...
    auto args = SM90HybridSparseWgmmaTmaRuntime::Args {
        .kernel = HybridSparseWgmmaTmaKernel::Dense,
    ...
    const auto dense_runtime = compiler->build(
        "sm90_hybrid_sparse_dense_wgmma_tma",
        SM90HybridSparseWgmmaTmaRuntime::generate(args));
    SM90HybridSparseWgmmaTmaRuntime::launch(dense_runtime, args);
    args.kernel = HybridSparseWgmmaTmaKernel::Sparse;
    args.launch_args = LaunchArgs(grid, 256, sparse_smem_bytes);
    const auto sparse_runtime = compiler->build(
        "sm90_hybrid_sparse_2_4_wgmma_tma",
        SM90HybridSparseWgmmaTmaRuntime::generate(args));
    SM90HybridSparseWgmmaTmaRuntime::launch(sparse_runtime, args);
    args.kernel = HybridSparseWgmmaTmaKernel::Reduce;
    args.launch_args = LaunchArgs(grid, 256);
    const auto reduce_runtime = compiler->build(
        "sm90_hybrid_sparse_reduce_wgmma_tma",
        SM90HybridSparseWgmmaTmaRuntime::generate(args));
    SM90HybridSparseWgmmaTmaRuntime::launch(reduce_runtime, args);
}
