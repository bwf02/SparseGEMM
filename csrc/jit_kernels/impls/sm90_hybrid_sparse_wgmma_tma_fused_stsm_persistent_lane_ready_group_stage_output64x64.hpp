...
static int get_group_stage_output64x64_num_stages(
        int block_n, int block_m);

class SM90HybridSparseGroupStageOutput64x64Runtime final:
        public LaunchRuntime<SM90HybridSparseGroupStageOutput64x64Runtime> {
public:
    using Args =
...
static void sm90_hybrid_block_sparse_bf16_gemm_group_stage_output64x64(
    ...
    const auto args = SM90HybridSparseGroupStageOutput64x64Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_group_stage_output64x64",
        SM90HybridSparseGroupStageOutput64x64Runtime::generate(args));
    SM90HybridSparseGroupStageOutput64x64Runtime::launch(runtime, args);
}
