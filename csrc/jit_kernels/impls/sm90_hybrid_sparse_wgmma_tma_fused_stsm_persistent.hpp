...
class SM90HybridSparseFusedStsmPersistentRuntime final:
        public LaunchRuntime<SM90HybridSparseFusedStsmPersistentRuntime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent(
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent",
        SM90HybridSparseFusedStsmPersistentRuntime::generate(args));
    SM90HybridSparseFusedStsmPersistentRuntime::launch(runtime, args);
}
