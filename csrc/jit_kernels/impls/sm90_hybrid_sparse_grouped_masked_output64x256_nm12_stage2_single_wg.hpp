...
class SM90HybridSparseGroupedMaskedOutput64x256NM12Stage2SingleWGRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupedMaskedOutput64x256NM12Stage2SingleWGRuntime> {
public:
    struct Args {
        void* block_selector;
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_grouped_masked_output64x256_nm12_stage2_single_wg.cuh>

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_grouped_masked_output64x256_nm12_stage2_single_wg<
            {}, {}, {}, {}, {}, {}, {}>);
    (void)ptr;
}}
...
static void sm90_hybrid_block_sparse_bf16_grouped_masked_output64x256_nm12_stage2_single_wg(
    ...
    DG_HOST_ASSERT(block_n == 1 and block_m == 2 and max_m == 64);
    DG_HOST_ASSERT(n % 256 == 0);
    constexpr int num_stages = 2;
    constexpr int output_bytes = 64 * 256 * sizeof(__nv_bfloat16);
    const int block_rows = n / 64;
    const int block_groups = k / 128;
    const int dense_count = block_m - block_n;
    const int stage_bytes =
        4 * dense_count * 64 * 64 * sizeof(__nv_bfloat16) +
        4 * block_n * 64 * 32 * sizeof(__nv_bfloat16) +
        64 * 128 * sizeof(__nv_bfloat16) +
        4 * block_n * 2 * 4 * 16 * sizeof(std::uint32_t);
    ...
    const auto tensor_map_activation = make_tma_2d_desc(
        a, k, num_experts * max_m, 128, 64, k, 128);
    ...
    const auto tensor_map_output = make_tma_cd_desc(
        d, num_experts * max_m, n, 64, 256, n, 1, 128);
    const int total_tiles = num_experts * (n / 256);
    const auto args = SM90HybridSparseGroupedMaskedOutput64x256NM12Stage2SingleWGRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_grouped_masked_output64x256_nm12_stage2_single_wg",
        SM90HybridSparseGroupedMaskedOutput64x256NM12Stage2SingleWGRuntime::generate(args));
    SM90HybridSparseGroupedMaskedOutput64x256NM12Stage2SingleWGRuntime::launch(runtime, args);
}
