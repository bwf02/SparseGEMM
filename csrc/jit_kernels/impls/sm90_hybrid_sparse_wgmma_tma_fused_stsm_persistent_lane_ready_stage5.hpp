...
class SM90HybridSparseFusedStsmPersistentLaneReadyStage5Runtime final:
        public LaunchRuntime<
            SM90HybridSparseFusedStsmPersistentLaneReadyStage5Runtime> {
public:
    struct Args {
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_stage5.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage5<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_stage5(
    ...
    constexpr int barrier_bytes = 10 * sizeof(std::uint64_t);
    constexpr int pipeline_bytes =
        5 * (64 * 64 + 64 * 64) * sizeof(__nv_bfloat16) + barrier_bytes;
    ...
    const int metadata_bytes = block_groups * block_n * 2 * 4 * 16 * sizeof(std::uint32_t);
    const int smem_bytes = output_offset + output_bytes + metadata_bytes;
    ...
    const auto args =
        SM90HybridSparseFusedStsmPersistentLaneReadyStage5Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage5",
        SM90HybridSparseFusedStsmPersistentLaneReadyStage5Runtime::generate(args));
    SM90HybridSparseFusedStsmPersistentLaneReadyStage5Runtime::launch(
        runtime, args);
}
