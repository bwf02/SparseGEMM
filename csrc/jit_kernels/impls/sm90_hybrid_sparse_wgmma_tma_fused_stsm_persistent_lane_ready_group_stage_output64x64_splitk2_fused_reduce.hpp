...
static int get_group_stage_output64x64_splitk2_fused_reduce_num_stages(
        int block_n, int block_m);

class SM90HybridSparseGroupStageOutput64x64SplitK2FusedReduceRuntime final:
        public LaunchRuntime<SM90HybridSparseGroupStageOutput64x64SplitK2FusedReduceRuntime> {
public:
    struct Args {
        void *block_selector, *hardware_metadata;
        void *partial, *tile_counters, *d;
    ...
    static std::string generate_impl(const Args& args) {
        return fmt::format(R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_splitk2_fused_reduce.cuh>

static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &hybrid_sparse_group_stage_output64x64_splitk2_fused_reduce<{}, {}, {}>);
    (void)ptr;
}}
)",
            args.block_n, args.block_m,
            get_group_stage_output64x64_splitk2_fused_reduce_num_stages(
                args.block_n, args.block_m));
    }
    ...
    static void launch_impl(const KernelHandle& kernel,
        ...
        DG_CUDA_UNIFIED_CHECK(launch_kernel(
            ...
            args.partial, args.tile_counters, args.d,
...
static void sm90_hybrid_block_sparse_bf16_gemm_group_stage_output64x64_splitk2_fused_reduce(
        const torch::Tensor& a, const torch::Tensor& block_selector,
        const torch::Tensor& dense_values, const torch::Tensor& sparse_values,
        const torch::Tensor& hardware_metadata, const torch::Tensor& partial,
        const torch::Tensor& tile_counters, const torch::Tensor& d,
    ...
    const int num_stages =
        get_group_stage_output64x64_splitk2_fused_reduce_num_stages(block_n, block_m);
    const int stage_bytes =
        dense_count * 64 * 64 * sizeof(__nv_bfloat16) +
        block_n * 64 * 32 * sizeof(__nv_bfloat16) +
        64 * 64 * block_m * sizeof(__nv_bfloat16) +
    ...
    const auto tensor_map_sparse = make_tma_2d_desc(
        ...
        32, 64, 32, 64);
    const auto tensor_map_output = tensor_map_activation;
    ...
    const int persistent_ctas = std::min(
        total_tasks, device_runtime->get_num_sms() * ctas_per_sm);
    const auto args = SM90HybridSparseGroupStageOutput64x64SplitK2FusedReduceRuntime::Args {
        .block_selector = block_selector.data_ptr(),
        .hardware_metadata = hardware_metadata.data_ptr(),
        .partial = partial.data_ptr(),
        .tile_counters = tile_counters.data_ptr(),
        .d = d.data_ptr(),
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_group_stage_output64x64_splitk2_fused_reduce",
        SM90HybridSparseGroupStageOutput64x64SplitK2FusedReduceRuntime::generate(args));
    SM90HybridSparseGroupStageOutput64x64SplitK2FusedReduceRuntime::launch(runtime, args);
}
