...
class SM90HybridSparseOutput128x64StageKindDescReuseRuntime final:
        public LaunchRuntime<SM90HybridSparseOutput128x64StageKindDescReuseRuntime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output128x64_stage_kind_desc_reuse.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output128x64_stage_kind_desc_reuse<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output128x64_stage_kind_desc_reuse(
    ...
    const auto args = SM90HybridSparseOutput128x64StageKindDescReuseRuntime::Args {
        ...
        .launch_args = LaunchArgs(persistent_ctas, 384, smem_bytes),
    };
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output128x64_stage_kind_desc_reuse",
        SM90HybridSparseOutput128x64StageKindDescReuseRuntime::generate(args));
    SM90HybridSparseOutput128x64StageKindDescReuseRuntime::launch(runtime, args);
}
