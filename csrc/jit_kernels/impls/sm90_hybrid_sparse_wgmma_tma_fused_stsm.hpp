...
class SM90HybridSparseFusedStsmRuntime final:
        public LaunchRuntime<SM90HybridSparseFusedStsmRuntime> {
public:
    struct Args {
...
static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_fused_wgmma_tma_stsm<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_wgmma_tma_fused_stsm(
    ...
    const auto args = SM90HybridSparseFusedStsmRuntime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_fused_wgmma_tma_stsm",
        SM90HybridSparseFusedStsmRuntime::generate(args));
    SM90HybridSparseFusedStsmRuntime::launch(runtime, args);
}
