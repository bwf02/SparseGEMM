...
class SM90HybridSparseOutput64x64StageKindAsyncGroup3Runtime final:
        public LaunchRuntime<SM90HybridSparseOutput64x64StageKindAsyncGroup3Runtime> {
public:
    using Args =
    ...
    static std::string generate_impl(const Args&) {
        return R"(
#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output64x64_stage_kind_async_group3.cuh>

static void __instantiate_kernel() {
    auto ptr = reinterpret_cast<void*>(&hybrid_sparse_output64x64_stage_kind_async_group3<>);
    (void)ptr;
}
)";
    }
...
static void sm90_hybrid_block_sparse_bf16_gemm_output64x64_stage_kind_async_group3(
    ...
    constexpr int stage_bytes =
        (64 * 64 + 64 * 64) * sizeof(__nv_bfloat16) +
    ...
    const auto args = SM90HybridSparseOutput64x64StageKindAsyncGroup3Runtime::Args {
    ...
    const auto runtime = compiler->build(
        "sm90_hybrid_sparse_output64x64_stage_kind_async_group3",
        SM90HybridSparseOutput64x64StageKindAsyncGroup3Runtime::generate(args));
    SM90HybridSparseOutput64x64StageKindAsyncGroup3Runtime::launch(runtime, args);
}
