...
// MoE baseline extension: map one fixed-shape SpMM instance to each grid.z
// slice while retaining Sputnik's original row swizzle and tile computation.
template <typename Config>
__global__ void __launch_bounds__(Config::kThreadsPerBlock)
    BatchedKernel(int m, int k, int n, int nonzeros,
  ...
  constexpr int elements_per_scalar = Config::kElementsPerScalar;
  SpmmKernel<Config>::KernelFn(
      m, k, n,
      row_indices + batch * m,
      values + batch * (nonzeros / elements_per_scalar),
      row_offsets + batch * (m + 1),
      column_indices + batch * (nonzeros / elements_per_scalar),
      dense_matrix + batch * (k * n / elements_per_scalar),
      /*bias=*/nullptr,
      out + batch * (m * n / elements_per_scalar));
}

template <typename Config>
cudaError_t CudaSpmmBatchedEx(
    int batch_count, int m, int k, int n, int nonzeros,
    const int* __restrict__ row_indices,
    const typename Config::ScalarValue* __restrict__ values,
    const int* __restrict__ row_offsets,
    const typename Config::ScalarIndex* __restrict__ column_indices,
  ...
  BatchedKernel<Config><<<grid_dim, block_dim, 0, stream>>>(
...
cudaError_t CudaSpmmBatched(
  ...
  if (batch_count <= 0 || n <= 0 || (n % 2) != 0 || (nonzeros % 2) != 0) {
    return cudaErrorInvalidValue;
  }
  if (n < 16) {
    using Config = SpmmConfig<half2, half8, half2, 4, 32, 8, 8, 4>;
    return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
        row_indices, values, row_offsets, column_indices, dense_matrix,
        output_matrix, stream);
  } else if (n == 16) {
    using Config = SpmmConfig<half2, half8, half2, 4, 32, 8, 8, 4, false>;
    return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
        row_indices, values, row_offsets, column_indices, dense_matrix,
        output_matrix, stream);
  } else if (n < 32) {
    using Config = SpmmConfig<half2, half8, half4, 4, 32, 16, 8, 4>;
    return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
        row_indices, values, row_offsets, column_indices, dense_matrix,
        output_matrix, stream);
  } else if (n == 32) {
    using Config = SpmmConfig<half2, half8, half4, 4, 32, 16, 8, 4, false>;
    return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
        row_indices, values, row_offsets, column_indices, dense_matrix,
        output_matrix, stream);
  } else if ((n % 64) == 0) {
    using Config = SpmmConfig<half2, half8, half8, 4, 32, 32, 8, 4, false>;
    return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
        row_indices, values, row_offsets, column_indices, dense_matrix,
        output_matrix, stream);
  }
  using Config = SpmmConfig<half2, half4, half4, 2, 32, 32, 16, 4>;
  return CudaSpmmBatchedEx<Config>(batch_count, m, k, n, nonzeros,
      row_indices, values, row_offsets, column_indices, dense_matrix,
      output_matrix, stream);
}
