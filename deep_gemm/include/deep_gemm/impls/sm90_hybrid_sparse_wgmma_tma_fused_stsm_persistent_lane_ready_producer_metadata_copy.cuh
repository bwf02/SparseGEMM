...
// Stage lane-ready metadata with a cooperative producer-warp vector copy.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesProducerMetadataCopy = 3;

__device__ __forceinline__ void advance_pipeline_producer_metadata_copy(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesProducerMetadataCopy - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_producer_metadata_copy(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesProducerMetadataCopy * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesProducerMetadataCopy * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesProducerMetadataCopy * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesProducerMetadataCopy;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);

    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesProducerMetadataCopy; ++stage) {
            ...
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    if (is_sparse) {
                        ...
                        reinterpret_cast<uint4*>(smem_metadata(producer_stage))[lane] =
                            reinterpret_cast<const uint4*>(hardware_metadata +
                                static_cast<long long>(packed_block) * 128)[lane];
                        __syncwarp();
                    ...
                    advance_pipeline_producer_metadata_copy(
...
#pragma unroll
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            if (thread_in_metadata_group < 2) {
                                ...
                                hardware_metadata = smem_metadata(consumer_stage)[
