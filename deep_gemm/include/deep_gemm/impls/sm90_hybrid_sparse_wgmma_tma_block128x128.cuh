...
// Hybrid sparse 128x128 weight blocks with a 64x128 output tile.
...
void hybrid_sparse_dense_wgmma_tma_block128x128(
...
#pragma unroll
                for (int k_tile = 0; k_tile < 8; ++k_tile) {
                    const int k_atom = k_tile / 4;
                    const int k_subtile = (k_tile % 4) * 16;
                    const auto desc_a = deep_gemm::mma::sm90::make_smem_desc(
                        smem_weight(stage) + k_atom * kBlockH * 64 +
                            n_tile * 64 * 64 + k_subtile,
                        static_cast<int>(cute::GMMA::LayoutType::B128), 0, 1024);
                    const auto desc_b = deep_gemm::mma::sm90::make_smem_desc(
                        smem_activation(stage) + k_atom * kTileM * 64 +
                            k_subtile,
                        static_cast<int>(cute::GMMA::LayoutType::B128), 0, 1024);
...
template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_2_4_wgmma_tma_block128x128(
...
template <int = 0>
__global__ void hybrid_sparse_reduce_wgmma_tma_block128x128(
