...
// Fixed-shape masked grouped 64x64 kernel with 2-CTA activation multicast with masked epilogue.
...
void hybrid_sparse_grouped_masked_output64x64_nm12_stage2_cluster2_masked_epilogue(
...
#pragma unroll
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        const int expert = tile_idx / tiles_n;
        const int tile_n = tile_idx % tiles_n;
        const int local_tile_m = 0;
...
#pragma unroll
                ...
                deep_gemm::ptx::warpgroup_wait<0>();
                if (lane < 2)
                    empty_barrier[consumer_stage].arrive(lane);
