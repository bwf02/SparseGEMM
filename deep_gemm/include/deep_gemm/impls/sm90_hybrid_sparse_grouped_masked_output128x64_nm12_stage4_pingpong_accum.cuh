...
// Persistent masked grouped 128x64 output tile with two WGMMA accumulator sets.
...
void hybrid_sparse_grouped_masked_output128x64_nm12_stage4_pingpong_accum(
...
#pragma unroll
        ...
        if (warp < 4) {
            ...
            float odd_accumulator[kAccumulatorCountProducerMetadataGroupStage128x64NM12DescReuseFusedMMAGroupFixedShapeUnrollKStage4AsyncGroup2] = {};
            int pending_stage[2] = {-1, -1};
#pragma unroll
            for (int block_group = 0; block_group < block_groups;
                 ++block_group) {
                const int accumulator_slot = block_group & 1;
                float* group_accumulator = accumulator_slot == 0
                    ? accumulator
                    : odd_accumulator;
                if (block_group >= 2) {
                    // Complete the previous user of this accumulator set before
                    // issuing another asynchronous WGMMA group into it.
                    deep_gemm::ptx::warpgroup_wait<1>();
                    release_stage(
                        &empty_barrier[pending_stage[accumulator_slot]]);
                }
...
#pragma unroll
                ...
                if constexpr (kBlockN == 1 && kBlockM == 2) {
                    ...
                    wgmma_group_stage_128x64_nm12_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage4_async_group2(
                        ...
                        group_accumulator, block_group >= 2);
                } else {
#pragma unroll
...
#pragma unroll
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            sparse_wgmma_group_stage_128x64_nm12_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage4_async_group2(
                                desc_a.desc_, desc_b.desc_, group_accumulator,
                                metadata,
                                block_group >= 2 || local_block != 0 ||
                                    k_tile != 0);
                        }
                    } else {
#pragma unroll
...
#pragma unroll
                for (int i = 0;
                    ...
                    deep_gemm::ptx::warpgroup_fence_operand(
                        group_accumulator[i]);
                pending_stage[accumulator_slot] = consumer_stage;
                advance_pipeline_group_stage_128x64_nm12_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage4_async_group2<kPipelineStages>(
                    consumer_stage, consumer_phase);
            }
            deep_gemm::ptx::warpgroup_wait<0>();
            release_stage(&empty_barrier[pending_stage[0]]);
            release_stage(&empty_barrier[pending_stage[1]]);

#pragma unroll
            for (int i = 0;
                 i < kAccumulatorCountProducerMetadataGroupStage128x64NM12DescReuseFusedMMAGroupFixedShapeUnrollKStage4AsyncGroup2; ++i)
                accumulator[i] += odd_accumulator[i];
