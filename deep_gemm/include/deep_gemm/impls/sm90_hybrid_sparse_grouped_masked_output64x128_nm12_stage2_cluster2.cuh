...
constexpr int kGroupedOutput64x128M = 64;
constexpr int kGroupedOutput64x128N = 128;
constexpr int kGroupedOutput64x128MathThreads = 256;
constexpr int kGroupedOutput64x128Threads = 384;
constexpr int kGroupedOutput64x128Accumulators = 32;

template <int kBlockN, int kBlockM, int kPipelineStages,
          int kNumExperts, int kMaxM, int kN, int kK>
__global__ __launch_bounds__(kGroupedOutput64x128Threads, 1)
void hybrid_sparse_grouped_masked_output64x128_nm12_stage2_cluster2(
    ...
    static_assert(kN % kGroupedOutput64x128N == 0 && kK % 128 == 0);
    ...
    constexpr int kActivationBytes =
        kGroupedOutput64x128M * kBlock * kBlockM * sizeof(__nv_bfloat16);
    ...
    constexpr int kOutputBytes =
        kGroupedOutput64x128M * kGroupedOutput64x128N * sizeof(__nv_bfloat16);
    ...
    constexpr int kTilesN = kN / kGroupedOutput64x128N;
...
#pragma unroll
    ...
    deep_gemm::comm::cluster_sync_with_relaxed_arrive();

    const int expert = static_cast<int>(blockIdx.x) / kTilesN;
    const int tile_n = static_cast<int>(blockIdx.x) % kTilesN;
...
#pragma unroll
            ...
            if (is_leader) {
                deep_gemm::tma::copy<128, 64, 128, cutlass::bfloat16_t>(
                    ...
                    block_group * 128, output_tile_m, 2);
...
#pragma unroll
    ...
    if (warp < 8) {
        auto run_math = [&]<int kWeightRow>() {
            float accumulator[kGroupedOutput64x128Accumulators] = {};
                    ...
                    constexpr auto activation_desc = [=](
                        ...
                        return make_constexpr_gmma_desc(
                            kActivationByteOffset +
                                local_block * kGroupedOutput64x128M * kBlock *
...
#pragma unroll
                    for (int i = 0; i < kGroupedOutput64x128Accumulators; ++i)
...
#pragma unroll
                    ...
                    if (lane < 2)
                        empty_barrier[kStage].arrive(lane);
                }(), ...);
...
#pragma unroll
        ...
        cutlass::arch::NamedBarrier::sync(kGroupedOutput64x128MathThreads, 1);
    }
}
