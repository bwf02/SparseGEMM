...
// Hybrid sparse 128x32 weight blocks with a 128x128 output tile.
...
constexpr int kWarpgroupTileM = 64;
constexpr int kReduceTileM = 64;
...
__device__ __forceinline__ void store_wgmma_accumulator(
    ...
    const int lane = static_cast<int>(threadIdx.x) & 31;
    const int warp = (static_cast<int>(threadIdx.x) >> 5) & 3;
...
void hybrid_sparse_dense_wgmma_tma_block128x32_output128x128(
                ...
                for (int k_tile = 0; k_tile < 2; ++k_tile) {
                    ...
                    const auto desc_b = deep_gemm::mma::sm90::make_smem_desc(
                        smem_activation(stage) +
                            math_wg * kWarpgroupTileM * kBlockW + k_tile * 16,
                        static_cast<int>(cute::GMMA::LayoutType::B64), 0, 512);
...
#pragma unroll
    ...
    const int warpgroup_output_m = output_tile_m + math_wg * kWarpgroupTileM;
    store_wgmma_accumulator(accumulator, dense_partial, warpgroup_output_m,
                            output_tile_n, m, n);
    store_wgmma_accumulator(accumulator + 32, dense_partial, warpgroup_output_m,
...
template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_2_4_wgmma_tma_block128x32_output128x128(
            ...
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    const int packed_block =
                        ...
                        sparse_slot;
                    deep_gemm::tma::copy<16, 128, 32, cutlass::bfloat16_t>(
...
#pragma unroll
                ...
                const auto desc_a = deep_gemm::mma::sm90::make_smem_desc(
                    smem_weight(stage) + n_tile * 64 * (kBlockW / 2),
                    static_cast<int>(cute::GMMA::LayoutType::B32), 0, 256);
                const auto desc_b = deep_gemm::mma::sm90::make_smem_desc(
                    smem_activation(stage) +
                        math_wg * kWarpgroupTileM * kBlockW,
                    static_cast<int>(cute::GMMA::LayoutType::B64), 0, 512);
                sparse_wgmma(desc_a.desc_, desc_b.desc_,
                             accumulator + n_tile * 32,
                             hardware_metadata, has_accumulator);
            }
            deep_gemm::ptx::warpgroup_commit_batch();
#pragma unroll
            for (int i = 0; i < 64; ++i)
    ...
    const int warpgroup_output_m = output_tile_m + math_wg * kWarpgroupTileM;
    store_wgmma_accumulator(accumulator, sparse_partial, warpgroup_output_m,
                            output_tile_n, m, n);
    store_wgmma_accumulator(accumulator + 32, sparse_partial, warpgroup_output_m,
                            output_tile_n + 64, m, n);
}


template <int = 0>
__global__ void hybrid_sparse_reduce_wgmma_tma_block128x32_output128x128(
