...
// Stage lane-ready metadata with TMA alongside each sparse weight tile.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesTmaMetadata = 3;

__device__ __forceinline__ void advance_pipeline_tma_metadata(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesTmaMetadata - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_tma_metadata(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesTmaMetadata * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesTmaMetadata * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesTmaMetadata * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesTmaMetadata;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);

    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesTmaMetadata; ++stage) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    if (is_sparse) {
                        ...
                        deep_gemm::tma::copy<32, 4, 0, unsigned>(
                            ...
                            0, packed_block * 4);
                        weight_bytes =
                            kSparseWeightBytes + kMetadataBytesPerBlock;
                        ++sparse_slot;
                    } else {
                        const int packed_block =
