...
// Small-M kernel: pair one 64x64 weight tile with a 64x64 activation tile.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kOutputTileMProducerMetadataCopy64x64 = 64;
constexpr int kOutputTileNProducerMetadataCopy64x64 = 64;
constexpr int kMathThreadsProducerMetadataCopy64x64 = 128;
constexpr int kThreadsProducerMetadataCopy64x64 = 256;
constexpr int kPipelineStagesProducerMetadataCopy64x64 = 3;
constexpr int kAccumulatorCountProducerMetadataCopy64x64 = 32;

using DenseMMAProducerMetadataCopy64x64 =
    typename deep_gemm::mma::sm90::BF16MMASelector<64>::type;

template <size_t... I>
__device__ __forceinline__ void sparse_wgmma_producer_metadata_copy_64x64_impl(
...
__device__ __forceinline__ void sparse_wgmma_producer_metadata_copy_64x64(
        const unsigned long long desc_a, const unsigned long long desc_b,
        float* accumulator, const unsigned metadata, const bool accumulate) {
    sparse_wgmma_producer_metadata_copy_64x64_impl(
        desc_a, desc_b, accumulator, metadata, accumulate,
        cute::make_index_sequence<kAccumulatorCountProducerMetadataCopy64x64>{});
}

__device__ __forceinline__ void advance_pipeline_producer_metadata_copy_64x64(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesProducerMetadataCopy64x64 - 1
        ? 0
        : stage + 1;
    phase ^= stage == 0;
}
...
template <int = 0>
__global__ __launch_bounds__(kThreadsProducerMetadataCopy64x64, 1)
void hybrid_sparse_output64x64_stage_kind(
    ...
    constexpr int kActivationBytes =
        kOutputTileMProducerMetadataCopy64x64 * kBlock *
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesProducerMetadataCopy64x64 * sizeof(Barrier);
    constexpr int kOutputBytes =
        kOutputTileMProducerMetadataCopy64x64 *
        kOutputTileNProducerMetadataCopy64x64 * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesProducerMetadataCopy64x64 * kStageBytes +
    ...
    const int tiles_m =
        (m + kOutputTileMProducerMetadataCopy64x64 - 1) /
        kOutputTileMProducerMetadataCopy64x64;
    const int tiles_n =
        (n + kOutputTileNProducerMetadataCopy64x64 - 1) /
        kOutputTileNProducerMetadataCopy64x64;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesProducerMetadataCopy64x64 * kStageBytes);
    auto empty_barrier =
        full_barrier + kPipelineStagesProducerMetadataCopy64x64;
    auto stage_control = reinterpret_cast<volatile unsigned long long*>(
        empty_barrier + kPipelineStagesProducerMetadataCopy64x64);
    auto smem_output =
...
#pragma unroll
        for (int stage = 0;
             stage < kPipelineStagesProducerMetadataCopy64x64; ++stage) {
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        const int output_tile_m =
            tile_m * kOutputTileMProducerMetadataCopy64x64;
        const int output_tile_n =
            tile_n * kOutputTileNProducerMetadataCopy64x64;
        ...
        if (warp == 6) {
            ...
            for (int block_group = 0; block_group < block_groups;
                ...
                for (int local_block = 0; local_block < block_m;
                    ...
                    advance_pipeline_producer_metadata_copy_64x64(
        ...
        if (warp < 4) {
            float accumulator[kAccumulatorCountProducerMetadataCopy64x64] = {};
...
#pragma unroll
                    for (int i = 0;
                         i < kAccumulatorCountProducerMetadataCopy64x64; ++i)
                        ...
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            const auto desc_b =
                                deep_gemm::mma::sm90::make_smem_desc(
                                    ...
                                    0, 1024);
                            sparse_wgmma_producer_metadata_copy_64x64(
                            ...
                            const auto desc_b =
                                deep_gemm::mma::sm90::make_smem_desc(
                                    ...
                                    static_cast<int>(
                                        cute::GMMA::LayoutType::B128),
                                    0, 1024);
                            DenseMMAProducerMetadataCopy64x64::wgmma(
                                desc_a.desc_, desc_b.desc_, accumulator,
                                has_accumulator || k_tile != 0);
                        }
                    }
...
#pragma unroll
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataCopy64x64, 0);
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataCopy64x64, 1);
        }
    }
}
