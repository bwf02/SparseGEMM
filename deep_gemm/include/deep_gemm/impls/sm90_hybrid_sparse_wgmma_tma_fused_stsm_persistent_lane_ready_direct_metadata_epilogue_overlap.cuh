...
// Direct-metadata kernel with producer overlap across the math warpgroup epilogue.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesDirectMetadataEpilogueOverlap = 3;

__device__ __forceinline__ void advance_pipeline_direct_metadata_epilogue_overlap(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesDirectMetadataEpilogueOverlap - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesDirectMetadataEpilogueOverlap * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesDirectMetadataEpilogueOverlap * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesDirectMetadataEpilogueOverlap * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesDirectMetadataEpilogueOverlap;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);

    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesDirectMetadataEpilogueOverlap; ++stage) {
        ...
        if (warp == 6 && cute::elect_one_sync()) {
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    advance_pipeline_direct_metadata_epilogue_overlap(
...
#pragma unroll
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            if (thread_in_metadata_group < 2) {
                                ...
                                hardware_metadata = global_metadata[
                                    metadata_base +
