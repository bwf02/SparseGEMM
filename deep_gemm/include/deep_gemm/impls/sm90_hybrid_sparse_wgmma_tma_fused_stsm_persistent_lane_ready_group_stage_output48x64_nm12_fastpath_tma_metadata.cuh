...
constexpr int kOutputTileMProducerMetadataGroupStage48x64NM12 = 48;
constexpr int kOutputTileNProducerMetadataGroupStage48x64NM12 = 64;
constexpr int kMathThreadsProducerMetadataGroupStage48x64NM12 = 128;
constexpr int kThreadsProducerMetadataGroupStage48x64NM12 = 256;
constexpr int kAccumulatorCountProducerMetadataGroupStage48x64NM12 = 24;

using DenseMMAProducerMetadataGroupStage48x64NM12 =
    typename deep_gemm::mma::sm90::BF16MMASelector<48>::type;

template <size_t... I>
__device__ __forceinline__ void sparse_wgmma_group_stage_48x64_nm12_impl(
...
__device__ __forceinline__ void sparse_wgmma_group_stage_48x64_nm12(
        const unsigned long long desc_a, const unsigned long long desc_b,
        float* accumulator, const unsigned metadata, const bool accumulate) {
    sparse_wgmma_group_stage_48x64_nm12_impl(
        desc_a, desc_b, accumulator, metadata, accumulate,
        cute::make_index_sequence<kAccumulatorCountProducerMetadataGroupStage48x64NM12>{});
}

template <int kPipelineStages>
__device__ __forceinline__ void advance_pipeline_group_stage_48x64_nm12(
...
__global__ __launch_bounds__(kThreadsProducerMetadataGroupStage48x64NM12, 1)
void hybrid_sparse_group_stage_output48x64_nm12_fastpath_tma_metadata(
        const long long* block_selector, const unsigned*,
        __nv_bfloat16*,
        ...
        const __grid_constant__ cute::TmaDescriptor tensor_map_metadata,
    ...
    constexpr int kActivationBytes =
        kOutputTileMProducerMetadataGroupStage48x64NM12 * kBlock * kBlockM *
    ...
    constexpr int kOutputBytes =
        kOutputTileMProducerMetadataGroupStage48x64NM12 *
        kOutputTileNProducerMetadataGroupStage48x64NM12 * sizeof(__nv_bfloat16);
    ...
    const int tiles_m =
        (m + kOutputTileMProducerMetadataGroupStage48x64NM12 - 1) /
        kOutputTileMProducerMetadataGroupStage48x64NM12;
    const int tiles_n =
        (n + kOutputTileNProducerMetadataGroupStage48x64NM12 - 1) /
        kOutputTileNProducerMetadataGroupStage48x64NM12;
...
#pragma unroll
    ...
    if (warp == 6 && cute::elect_one_sync()) {
        ...
        cute::prefetch_tma_descriptor(&tensor_map_metadata);
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        const int output_tile_m =
            tile_m * kOutputTileMProducerMetadataGroupStage48x64NM12;
        const int output_tile_n =
            tile_n * kOutputTileNProducerMetadataGroupStage48x64NM12;
...
#pragma unroll
                for (int sparse_slot = 0; sparse_slot < kBlockN;
                    ...
                    if (is_leader) {
                        deep_gemm::tma::copy<
                            ...
                            0, packed_block * kBlock);
                        deep_gemm::tma::copy<32, 4, 0, unsigned>(
                            &tensor_map_metadata,
                            &full_barrier[producer_stage],
                ...
                if (is_leader) {
                    deep_gemm::tma::copy<
                        kBlock * kBlockM,
                        kOutputTileMProducerMetadataGroupStage48x64NM12, 128,
                        cutlass::bfloat16_t>(
                ...
                advance_pipeline_group_stage_48x64_nm12<kPipelineStages>(
                    producer_stage, producer_phase);
            }
        }

        if (warp < 4) {
            float accumulator[kAccumulatorCountProducerMetadataGroupStage48x64NM12] = {};
...
#pragma unroll
                for (int i = 0;
                     i < kAccumulatorCountProducerMetadataGroupStage48x64NM12; ++i)
                    ...
                    for (int k_tile = 0; k_tile < 2; ++k_tile) {
                        ...
                        const auto desc_b =
                            deep_gemm::mma::sm90::make_smem_desc(
                                ...
                                0, 1024);
                        sparse_wgmma_group_stage_48x64_nm12(
                        ...
                        const auto desc_b =
                            deep_gemm::mma::sm90::make_smem_desc(
                                ...
                                static_cast<int>(
                                    cute::GMMA::LayoutType::B128),
                                0, 1024);
                        DenseMMAProducerMetadataGroupStage48x64NM12::wgmma(
...
#pragma unroll
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataGroupStage48x64NM12, 0);
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataGroupStage48x64NM12, 1);
        }
    }
}
