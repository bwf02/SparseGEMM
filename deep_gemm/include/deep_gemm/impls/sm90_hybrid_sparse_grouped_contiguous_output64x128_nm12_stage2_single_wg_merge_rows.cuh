...
constexpr int kGroupedContiguousOutput64x128SingleWGMergeRowsM = 64;
constexpr int kGroupedContiguousOutput64x128SingleWGMergeRowsN = 128;
constexpr int kGroupedContiguousOutput64x128SingleWGMergeRowsMathThreads = 128;
constexpr int kGroupedContiguousOutput64x128SingleWGMergeRowsThreads = 256;
constexpr int kGroupedContiguousOutput64x128SingleWGMergeRowsAccumulators = 32;

template <int kBlockN, int kBlockM, int kPipelineStages,
          int kNumExperts, int kTotalM, int kN, int kK>
__global__ __launch_bounds__(kGroupedContiguousOutput64x128SingleWGMergeRowsThreads, 1)
void hybrid_sparse_grouped_contiguous_output64x128_nm12_stage2_single_wg_merge_rows(
    ...
    static_assert(kNumExperts > 0 && kTotalM > 0 && kTotalM % 64 == 0);
    static_assert(kN % kGroupedContiguousOutput64x128SingleWGMergeRowsN == 0 && kK % 128 == 0);
    ...
    constexpr int kActivationBytes =
        kGroupedContiguousOutput64x128SingleWGMergeRowsM * kBlock * kBlockM * sizeof(__nv_bfloat16);
    ...
    constexpr int kOutputBytes =
        kGroupedContiguousOutput64x128SingleWGMergeRowsM * kGroupedContiguousOutput64x128SingleWGMergeRowsN * sizeof(__nv_bfloat16);
    ...
    constexpr int kTilesM = kTotalM / kGroupedContiguousOutput64x128SingleWGMergeRowsM;
    constexpr int kTilesN = kN / kGroupedContiguousOutput64x128SingleWGMergeRowsN;
...
#pragma unroll
    ...
    const int output_tile_m =
        tile_m * kGroupedContiguousOutput64x128SingleWGMergeRowsM;
    const int output_tile_n = tile_n * kGroupedContiguousOutput64x128SingleWGMergeRowsN;
    int expert = 0;
    int valid_rows = 0;
    int previous_end = 0;
#pragma unroll
    ...
    if (valid_rows == 0) {
        for (int index = static_cast<int>(threadIdx.x);
             index < kGroupedContiguousOutput64x128SingleWGMergeRowsM *
                         kGroupedContiguousOutput64x128SingleWGMergeRowsN;
             index += kGroupedContiguousOutput64x128SingleWGMergeRowsThreads)
...
#pragma unroll
    ...
    if (warp < 4) {
        float accumulator[kWeightRows][kGroupedContiguousOutput64x128SingleWGMergeRowsAccumulators] = {};
...
#pragma unroll
                    for (int i = 0;
                         i < kGroupedContiguousOutput64x128SingleWGMergeRowsAccumulators;
                         ++i)
                    ...
                    constexpr auto activation_desc = [=](
                        ...
                        return make_constexpr_gmma_desc(
                            kActivationByteOffset +
                                local_block * kGroupedContiguousOutput64x128SingleWGMergeRowsM * kBlock *
...
#pragma unroll
                for (int weight_row = 0; weight_row < kWeightRows; ++weight_row) {
#pragma unroll
                    for (int i = 0; i < kGroupedContiguousOutput64x128SingleWGMergeRowsAccumulators; ++i) {
                        deep_gemm::ptx::warpgroup_fence_operand(
                            accumulator[weight_row][i]);
                    }
                }
...
#pragma unroll
        ...
        cute::tma_store_fence();
        cutlass::arch::NamedBarrier::sync(kGroupedContiguousOutput64x128SingleWGMergeRowsMathThreads, 0);
...
#pragma unroll
        ...
        cutlass::arch::NamedBarrier::sync(kGroupedContiguousOutput64x128SingleWGMergeRowsMathThreads, 1);
    }
}
