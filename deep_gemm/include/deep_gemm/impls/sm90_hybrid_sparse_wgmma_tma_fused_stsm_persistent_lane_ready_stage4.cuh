...
// Persistent lane-ready hybrid sparse kernel with a four-stage TMA pipeline.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesStage4 = 4;

__device__ __forceinline__ void advance_pipeline_stage4(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesStage4 - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage4(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesStage4 * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesStage4 * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesStage4 * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesStage4;
    ...
    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesStage4; ++stage) {
        ...
        if (warp == 6 && cute::elect_one_sync()) {
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    advance_pipeline_stage4(producer_stage, producer_phase);
                }
            }
        }
...
#pragma unroll
                    ...
                    advance_pipeline_stage4(consumer_stage, consumer_phase);
                }
            }
