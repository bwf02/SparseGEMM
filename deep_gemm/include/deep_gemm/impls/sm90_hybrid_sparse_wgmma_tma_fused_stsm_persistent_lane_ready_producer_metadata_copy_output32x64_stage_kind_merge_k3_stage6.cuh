...
// Small-M kernel with three K blocks per WGMMA group and six TMA stages.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kOutputTileMProducerMetadataCopy32x64MergeK3Stage6 = 32;
constexpr int kOutputTileNProducerMetadataCopy32x64MergeK3Stage6 = 64;
constexpr int kMathThreadsProducerMetadataCopy32x64MergeK3Stage6 = 128;
constexpr int kThreadsProducerMetadataCopy32x64MergeK3Stage6 = 256;
constexpr int kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6 = 6;
constexpr int kMergedKStagesProducerMetadataCopy32x64MergeK3Stage6 = 3;
constexpr int kAccumulatorCountProducerMetadataCopy32x64MergeK3Stage6 = 16;

using DenseMMAProducerMetadataCopy32x64MergeK3Stage6 =
    typename deep_gemm::mma::sm90::BF16MMASelector<32>::type;

template <size_t... I>
__device__ __forceinline__ void sparse_wgmma_producer_metadata_copy_32x64_merge_k3_stage6_impl(
...
__device__ __forceinline__ void sparse_wgmma_producer_metadata_copy_32x64_merge_k3_stage6(
        const unsigned long long desc_a, const unsigned long long desc_b,
        float* accumulator, const unsigned metadata, const bool accumulate) {
    sparse_wgmma_producer_metadata_copy_32x64_merge_k3_stage6_impl(
        desc_a, desc_b, accumulator, metadata, accumulate,
        cute::make_index_sequence<kAccumulatorCountProducerMetadataCopy32x64MergeK3Stage6>{});
}

__device__ __forceinline__ void advance_pipeline_producer_metadata_copy_32x64_merge_k3_stage6(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6 - 1
        ? 0
        : stage + 1;
    phase ^= stage == 0;
}
...
template <int = 0>
__global__ __launch_bounds__(kThreadsProducerMetadataCopy32x64MergeK3Stage6, 1)
void hybrid_sparse_output32x64_stage_kind_merge_k3_stage6(
    ...
    constexpr int kActivationBytes =
        kOutputTileMProducerMetadataCopy32x64MergeK3Stage6 * kBlock *
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6 * sizeof(Barrier);
    constexpr int kOutputBytes =
        kOutputTileMProducerMetadataCopy32x64MergeK3Stage6 *
        kOutputTileNProducerMetadataCopy32x64MergeK3Stage6 * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6 * kStageBytes +
    ...
    const int tiles_m =
        (m + kOutputTileMProducerMetadataCopy32x64MergeK3Stage6 - 1) /
        kOutputTileMProducerMetadataCopy32x64MergeK3Stage6;
    const int tiles_n =
        (n + kOutputTileNProducerMetadataCopy32x64MergeK3Stage6 - 1) /
        kOutputTileNProducerMetadataCopy32x64MergeK3Stage6;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6 * kStageBytes);
    auto empty_barrier =
        full_barrier + kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6;
    auto stage_control = reinterpret_cast<volatile unsigned long long*>(
        empty_barrier + kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6);
    auto smem_output =
...
#pragma unroll
        for (int stage = 0;
             stage < kPipelineStagesProducerMetadataCopy32x64MergeK3Stage6; ++stage) {
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        const int output_tile_m =
            tile_m * kOutputTileMProducerMetadataCopy32x64MergeK3Stage6;
        const int output_tile_n =
            tile_n * kOutputTileNProducerMetadataCopy32x64MergeK3Stage6;
        ...
        if (warp == 6) {
            ...
            for (int block_group = 0; block_group < block_groups;
                ...
                for (int local_block = 0; local_block < block_m;
                    ...
                    advance_pipeline_producer_metadata_copy_32x64_merge_k3_stage6(
        ...
        if (warp < 4) {
            float accumulator[kAccumulatorCountProducerMetadataCopy32x64MergeK3Stage6] = {};
            bool has_accumulator = false;
            int pending_stages[kMergedKStagesProducerMetadataCopy32x64MergeK3Stage6];
            int pending_count = 0;
            for (int block_group = 0; block_group < block_groups;
                 ++block_group) {
                for (int local_block = 0; local_block < block_m;
...
#pragma unroll
                        for (int i = 0;
                             i < kAccumulatorCountProducerMetadataCopy32x64MergeK3Stage6;
                             ++i) {
                        ...
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            const auto desc_b =
                                deep_gemm::mma::sm90::make_smem_desc(
                                    ...
                                    0, 1024);
                            sparse_wgmma_producer_metadata_copy_32x64_merge_k3_stage6(
...
#pragma unroll
                            ...
                            const auto desc_b =
                                deep_gemm::mma::sm90::make_smem_desc(
                                    ...
                                    static_cast<int>(
                                        cute::GMMA::LayoutType::B128),
                                    0, 1024);
                            DenseMMAProducerMetadataCopy32x64MergeK3Stage6::wgmma(
                                desc_a.desc_, desc_b.desc_, accumulator,
                                has_accumulator || k_tile != 0);
                        }
                    }
                    ...
                    const bool is_last_block =
                        block_group == block_groups - 1 &&
                        local_block == block_m - 1;
                    if (pending_count ==
                            kMergedKStagesProducerMetadataCopy32x64MergeK3Stage6 ||
                        is_last_block) {
...
#pragma unroll
                        for (int pending = 0;
                             pending < kMergedKStagesProducerMetadataCopy32x64MergeK3Stage6;
                             ++pending) {
...
#pragma unroll
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataCopy32x64MergeK3Stage6, 0);
            ...
            cutlass::arch::NamedBarrier::sync(
                kMathThreadsProducerMetadataCopy32x64MergeK3Stage6, 1);
        }
    }
}
