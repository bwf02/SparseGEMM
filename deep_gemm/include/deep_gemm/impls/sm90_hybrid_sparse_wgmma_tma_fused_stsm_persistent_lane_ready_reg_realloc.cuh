...
// Persistent lane-ready hybrid sparse kernel with Stage 3 TMA and warpgroup register reallocation.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>
#include <cutlass/arch/reg_reconfig.h>

constexpr int kPipelineStagesRegRealloc = 3;
constexpr int kProducerRegistersRegRealloc = 40;
constexpr int kMathRegistersRegRealloc = 128;

__device__ __forceinline__ void advance_pipeline_reg_realloc(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesRegRealloc - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_reg_realloc(
    ...
    constexpr int kMetadataWordsPerBlock = 2 * 4 * 16;
    constexpr int kBarrierBytes =
        2 * kPipelineStagesRegRealloc * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesRegRealloc * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    const int metadata_words = block_groups * block_n * kMetadataWordsPerBlock;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesRegRealloc * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesRegRealloc;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);
    auto smem_metadata = reinterpret_cast<unsigned*>(
        smem + kOutputOffset + kOutputBytes);

    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesRegRealloc; ++stage) {
    ...
    if (warp >= 4) {
        cutlass::arch::warpgroup_reg_dealloc<kProducerRegistersRegRealloc>();
    } else {
        cutlass::arch::warpgroup_reg_alloc<kMathRegistersRegRealloc>();
    }
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        const auto* global_metadata = hardware_metadata +
            static_cast<long long>(block_row) * metadata_words;
        for (int word = static_cast<int>(threadIdx.x);
             word < metadata_words; word += kThreads) {
            smem_metadata[word] = global_metadata[word];
        }
        __syncthreads();
        ...
        if (warp == 6 && cute::elect_one_sync()) {
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    advance_pipeline_reg_realloc(producer_stage, producer_phase);
                }
            }
        }
...
#pragma unroll
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            ...
                            if (thread_in_metadata_group < 2) {
                                ...
                                hardware_metadata = smem_metadata[
                                    metadata_base +
                                    (k_tile * 4 + warp) * 16 + active_lane];
                            }
...
#pragma unroll
                    ...
                    advance_pipeline_reg_realloc(consumer_stage, consumer_phase);
                }
            }
