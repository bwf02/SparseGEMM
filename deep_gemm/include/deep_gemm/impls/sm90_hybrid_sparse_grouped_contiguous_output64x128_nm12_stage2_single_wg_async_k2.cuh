...
constexpr int kGroupedContiguousOutput64x128SingleWGAsyncK2M = 64;
constexpr int kGroupedContiguousOutput64x128SingleWGAsyncK2N = 128;
constexpr int kGroupedContiguousOutput64x128SingleWGAsyncK2MathThreads = 128;
constexpr int kGroupedContiguousOutput64x128SingleWGAsyncK2Threads = 256;
constexpr int kGroupedContiguousOutput64x128SingleWGAsyncK2Accumulators = 32;

template <int kBlockN, int kBlockM, int kPipelineStages,
          int kNumExperts, int kTotalM, int kN, int kK>
__global__ __launch_bounds__(kGroupedContiguousOutput64x128SingleWGAsyncK2Threads, 1)
void hybrid_sparse_grouped_contiguous_output64x128_nm12_stage2_single_wg_async_k2(
    ...
    static_assert(kN % kGroupedContiguousOutput64x128SingleWGAsyncK2N == 0 && kK % 128 == 0);
    ...
    constexpr int kActivationBytes =
        kGroupedContiguousOutput64x128SingleWGAsyncK2M * kBlock * kBlockM * sizeof(__nv_bfloat16);
    ...
    constexpr int kOutputBytes =
        kGroupedContiguousOutput64x128SingleWGAsyncK2M * kGroupedContiguousOutput64x128SingleWGAsyncK2N * sizeof(__nv_bfloat16);
    ...
    constexpr int kTilesM = kTotalM / kGroupedContiguousOutput64x128SingleWGAsyncK2M;
    constexpr int kTilesN = kN / kGroupedContiguousOutput64x128SingleWGAsyncK2N;
...
#pragma unroll
    ...
    const int output_tile_m =
        tile_m * kGroupedContiguousOutput64x128SingleWGAsyncK2M;
    const int output_tile_n = tile_n * kGroupedContiguousOutput64x128SingleWGAsyncK2N;
    int expert = 0;
    int valid_rows = 0;
    int previous_end = 0;
#pragma unroll
    ...
    if (valid_rows == 0) {
        for (int index = static_cast<int>(threadIdx.x);
             index < kGroupedContiguousOutput64x128SingleWGAsyncK2M *
                         kGroupedContiguousOutput64x128SingleWGAsyncK2N;
             index += kGroupedContiguousOutput64x128SingleWGAsyncK2Threads)
...
#pragma unroll
    ...
    if (warp < 4) {
        float accumulator[kWeightRows][kGroupedContiguousOutput64x128SingleWGAsyncK2Accumulators] = {};
        int pending_stage = -1;
                    ...
                    constexpr auto activation_desc = [=](
                        ...
                        return make_constexpr_gmma_desc(
                            kActivationByteOffset +
                                local_block * kGroupedContiguousOutput64x128SingleWGAsyncK2M * kBlock *
...
#pragma unroll
                    for (int i = 0; i < kGroupedContiguousOutput64x128SingleWGAsyncK2Accumulators; ++i)
...
#pragma unroll
        ...
        cute::tma_store_fence();
        cutlass::arch::NamedBarrier::sync(kGroupedContiguousOutput64x128SingleWGAsyncK2MathThreads, 0);
...
#pragma unroll
        ...
        cutlass::arch::NamedBarrier::sync(kGroupedContiguousOutput64x128SingleWGAsyncK2MathThreads, 1);
    }
}
