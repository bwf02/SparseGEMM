...
// Fixed-shape masked grouped 64x64 kernel with four-CTA shared-memory sizing.
...
constexpr int kStagedMetadataWordsOutputReusePartialMetadata = 62;

__device__ __forceinline__ bool is_direct_metadata_word_output_reuse_partial_metadata(
        const int logical_index) {
    return logical_index <= 32 ||
           (logical_index >= 64 && logical_index <= 96);
}

__device__ __forceinline__ int staged_metadata_index_output_reuse_partial_metadata(
        const int logical_index) {
...
void hybrid_sparse_grouped_masked_output64x64_nm12_fixed_stage2_output_reuse_partial_metadata(
    ...
    constexpr int kMetadataBytes =
        kStagedMetadataWordsOutputReusePartialMetadata * sizeof(unsigned);
    constexpr int kSelectorBytes = sizeof(unsigned long long);
    constexpr int kStageBytes =
        kDenseWeightBytes + kSparseWeightBytes +
        kActivationBytes + kMetadataBytes + kSelectorBytes;
    static_assert(kStageBytes % 128 == 0);
    constexpr int kBarrierBytes =
        2 * kPipelineStages * sizeof(Barrier);
    static_assert(
        kPipelineStages * kStageBytes + kBarrierBytes == 57888);
    ...
    auto smem_selector = [&](const int stage) {
...
#pragma unroll
            for (int block_group = 0; block_group < block_groups;
                ...
                if (is_leader)
                    *smem_selector(producer_stage) = selector;
#pragma unroll
...
#pragma unroll
                    for (int logical_index = lane; logical_index < 128;
                         logical_index += 32) {
                        if (!is_direct_metadata_word_output_reuse_partial_metadata(
                                logical_index)) {
                            const int staged_index =
                                staged_metadata_index_output_reuse_partial_metadata(
                                    logical_index);
                            smem_metadata(producer_stage)[staged_index] =
                                hardware_metadata[
                                    static_cast<long long>(packed_block) * 128 +
                                    logical_index];
                        }
                    }
                }
            ...
            if constexpr (kBlockN == 1 && kBlockM == 2) {
                [&]<size_t... Group>(cute::index_sequence<Group...>) {
                    ([&] {
                        ...
                        const unsigned long long selector =
                            *smem_selector(kStage);
                        constexpr long long kMetadataBlockOffset =
                            static_cast<long long>(kGroup) * 128;
                        const long long metadata_base =
                            ((static_cast<long long>(expert) *
                              (kN / kBlock) + block_row) * block_groups *
                             128) + kMetadataBlockOffset;
                        ...
                        if (thread_in_metadata_group < 2) {
                            const int logical_index_0 =
                                warp_in_math_wg * 16 + active_lane;
                            const int logical_index_1 =
                                (4 + warp_in_math_wg) * 16 + active_lane;
                            const bool load_direct =
                                warp_in_math_wg < 2 ||
                                (warp_in_math_wg == 2 && active_lane == 0);
                            metadata_0 = load_direct
                                ? hardware_metadata[
                                      metadata_base + logical_index_0]
                                : smem_metadata(kStage)[
                                      staged_metadata_index_output_reuse_partial_metadata(
                                          logical_index_0)];
                            metadata_1 = load_direct
                                ? hardware_metadata[
                                      metadata_base + logical_index_1]
                                : smem_metadata(kStage)[
                                      staged_metadata_index_output_reuse_partial_metadata(
                                          logical_index_1)];
                        }
