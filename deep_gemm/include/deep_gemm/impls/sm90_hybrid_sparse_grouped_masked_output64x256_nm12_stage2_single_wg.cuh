...
constexpr int kGroupedOutput64x256M = 64;
constexpr int kGroupedOutput64x256N = 256;
constexpr int kGroupedOutput64x256MathThreads = 128;
constexpr int kGroupedOutput64x256Threads = 256;
constexpr int kGroupedOutput64x256Accumulators = 32;

template <int kBlockN, int kBlockM, int kPipelineStages,
          int kNumExperts, int kMaxM, int kN, int kK>
__global__ __launch_bounds__(kGroupedOutput64x256Threads, 1)
void hybrid_sparse_grouped_masked_output64x256_nm12_stage2_single_wg(
    ...
    static_assert(kN % kGroupedOutput64x256N == 0 && kK % 128 == 0);
    if (block_n != kBlockN || block_m != kBlockM ||
        num_experts != kNumExperts || max_m != kMaxM ||
        n != kN || k != kK)
        return;

    constexpr int kWeightRows = 4;
    ...
    constexpr int kActivationBytes =
        kGroupedOutput64x256M * kBlock * kBlockM * sizeof(__nv_bfloat16);
    ...
    constexpr int kOutputBytes =
        kGroupedOutput64x256M * kGroupedOutput64x256N * sizeof(__nv_bfloat16);
    ...
    constexpr int kTilesN = kN / kGroupedOutput64x256N;
...
#pragma unroll
    ...
    const int output_tile_n = tile_n * kGroupedOutput64x256N;
...
#pragma unroll
    ...
    if (warp < 4) {
        float accumulator[kWeightRows][kGroupedOutput64x256Accumulators] = {};
                    ...
                    constexpr auto activation_desc = [=](
                        ...
                        return make_constexpr_gmma_desc(
                            kActivationByteOffset +
                                local_block * kGroupedOutput64x256M * kBlock *
...
#pragma unroll
                    for (int i = 0; i < kGroupedOutput64x256Accumulators; ++i)
                ...
                issue_weight_row.template operator()<2>();
                issue_weight_row.template operator()<3>();
#pragma unroll
                for (int weight_row = 0; weight_row < kWeightRows; ++weight_row) {
#pragma unroll
                    for (int i = 0; i < kGroupedOutput64x256Accumulators; ++i) {
...
#pragma unroll
        ...
        cutlass::arch::NamedBarrier::sync(kGroupedOutput64x256MathThreads, 0);
        if (warp == 0 && cute::elect_one_sync()) {
#pragma unroll
            for (int output_atom = 0; output_atom < 4; ++output_atom) {
        ...
        cutlass::arch::NamedBarrier::sync(kGroupedOutput64x256MathThreads, 1);
    }
}
