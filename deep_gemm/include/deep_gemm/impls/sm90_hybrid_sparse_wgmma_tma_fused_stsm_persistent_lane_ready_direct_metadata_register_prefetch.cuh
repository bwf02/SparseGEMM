...
// Direct-metadata kernel with register prefetch before each TMA stage wait.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesDirectMetadataRegisterPrefetch = 3;

__device__ __forceinline__ void advance_pipeline_direct_metadata_register_prefetch(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesDirectMetadataRegisterPrefetch - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_register_prefetch(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesDirectMetadataRegisterPrefetch * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesDirectMetadataRegisterPrefetch * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesDirectMetadataRegisterPrefetch * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesDirectMetadataRegisterPrefetch;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);

    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesDirectMetadataRegisterPrefetch; ++stage) {
    ...
    for (int tile_idx = static_cast<int>(blockIdx.x);
        ...
        if (warp == 6 && cute::elect_one_sync()) {
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    advance_pipeline_direct_metadata_register_prefetch(
        ...
        if (warp < 4) {
            ...
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    const bool is_sparse = (selector >> local_block) & 1ULL;
                    unsigned prefetched_metadata[2] = {};
                    if (is_sparse && thread_in_metadata_group < 2) {
...
#pragma unroll
                        for (int k_tile = 0; k_tile < 2; ++k_tile) {
                            prefetched_metadata[k_tile] = global_metadata[
                                metadata_base +
...
#pragma unroll
            ...
            cutlass::arch::NamedBarrier::sync(kMathThreads, 1);
        }
    }
}
