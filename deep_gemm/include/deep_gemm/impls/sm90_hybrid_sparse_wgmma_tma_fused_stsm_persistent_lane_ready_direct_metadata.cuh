...
// Persistent lane-ready hybrid sparse kernel with direct global lane-ready metadata loads.

#include <deep_gemm/impls/sm90_hybrid_sparse_wgmma_tma_fused_stsm.cuh>

constexpr int kPipelineStagesDirectMetadata = 3;

__device__ __forceinline__ void advance_pipeline_direct_metadata(
        int& stage, unsigned& phase) {
    stage = stage == kPipelineStagesDirectMetadata - 1 ? 0 : stage + 1;
    phase ^= stage == 0;
}

template <int = 0>
__global__ __launch_bounds__(kThreads, 1)
void hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata(
    ...
    constexpr int kBarrierBytes =
        2 * kPipelineStagesDirectMetadata * sizeof(Barrier);
    constexpr int kOutputBytes = kBlock * kBlock * sizeof(__nv_bfloat16);
    constexpr int kOutputOffset =
        ((kPipelineStagesDirectMetadata * kStageBytes + kBarrierBytes + 1023) / 1024) * 1024;
    ...
    auto full_barrier = reinterpret_cast<Barrier*>(
        smem + kPipelineStagesDirectMetadata * kStageBytes);
    auto empty_barrier = full_barrier + kPipelineStagesDirectMetadata;
    auto smem_output = reinterpret_cast<__nv_bfloat16*>(smem + kOutputOffset);
    if (warp == 4 && lane == 0) {
#pragma unroll
        for (int stage = 0; stage < kPipelineStagesDirectMetadata; ++stage) {
        ...
        if (warp == 6 && cute::elect_one_sync()) {
            for (int block_group = 0; block_group < block_groups; ++block_group) {
                ...
                for (int local_block = 0; local_block < block_m; ++local_block) {
                    ...
                    advance_pipeline_direct_metadata(producer_stage, producer_phase);
                }
            }
        }
...
#pragma unroll
                    ...
                    advance_pipeline_direct_metadata(consumer_stage, consumer_phase);
                }
            }
