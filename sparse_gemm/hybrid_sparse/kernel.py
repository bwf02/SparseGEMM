...
def hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group(
    a: torch.Tensor,
    packed_weight: HybridBlockSparseWeight,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Run output128 with one fused mixed sparse/dense WGMMA issue group."""
    if (packed_weight.layout.block_n, packed_weight.layout.block_m) != (1, 2):
        raise ValueError("output128 fused MMA group requires N:M=1:2")
    metadata = packed_weight.hardware_metadata
    if metadata is None:
        raise ValueError(
            "packed_weight does not contain lane-ready hardware metadata"
...
def hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group_fixed_shape_unroll_k(
    a: torch.Tensor,
    packed_weight: HybridBlockSparseWeight,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Run the output128 fused group with exact-shape K unrolling."""
    if (packed_weight.layout.block_n, packed_weight.layout.block_m) != (1, 2):
        raise ValueError("output128 fixed-shape fused MMA group requires N:M=1:2")
    metadata = packed_weight.hardware_metadata
    if metadata is None:
        raise ValueError(
            "packed_weight does not contain lane-ready hardware metadata"
...
def hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage5_output_reuse(
    a: torch.Tensor,
    packed_weight: HybridBlockSparseWeight,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Run exact-shape K unrolling with five TMA stages and stage-0 output reuse."""
    if (packed_weight.layout.block_n, packed_weight.layout.block_m) != (1, 2):
        raise ValueError("output128 stage-5 fused MMA group requires N:M=1:2")
    metadata = packed_weight.hardware_metadata
    if metadata is None:
        raise ValueError(
            "packed_weight does not contain lane-ready hardware metadata"
...
def hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage5_async_group2(
    a: torch.Tensor,
    packed_weight: HybridBlockSparseWeight,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Run five TMA stages while keeping one committed WGMMA group in flight."""
    if (packed_weight.layout.block_n, packed_weight.layout.block_m) != (1, 2):
        raise ValueError("output128 stage-5 async group requires N:M=1:2")
    metadata = packed_weight.hardware_metadata
    if metadata is None:
        raise ValueError(
            "packed_weight does not contain lane-ready hardware metadata"
...
def hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage4_async_group2(
    a: torch.Tensor,
    packed_weight: HybridBlockSparseWeight,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """Run four TMA stages while keeping one committed WGMMA group in flight."""
    if (packed_weight.layout.block_n, packed_weight.layout.block_m) != (1, 2):
        raise ValueError("output128 stage-4 async group requires N:M=1:2")
    metadata = packed_weight.hardware_metadata
    if metadata is None:
        raise ValueError(
            "packed_weight does not contain lane-ready hardware metadata"
...
def _select_hybrid_block_sparse_gemm_wgmma_tuned(
    ...
    if (layout.block_n, layout.block_m) == (1, 2):
        tuned = {
            ...
            (256, 2048, 1408): hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output128x64_nm12_fastpath_desc_reuse_fused_mma_group_fixed_shape_unroll_k_stage4_async_group2,
            ...
            (2048, 1408, 2048): hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_producer_metadata_copy_output128x64_stage_kind,
...
def hybrid_block_sparse_grouped_masked_wgmma_tma(
    ...
    ``expected_m`` is a performance-only dispatch hint for fixed-capacity
    kernels. Large prefill kernels select their N tile on the device from the
    current rank's ``masked_m`` values. The hint defaults to the masked
    activation capacity and does not constrain values in ``masked_m``.
    """
    experts, n, _ = _validate_grouped_inputs(a, packed_weight, masked_m, 3)
    metadata = _require_grouped_hardware_metadata(a, packed_weight)
    if a.shape[0] != experts:
