"""Compare epilogue overlap with register metadata prefetch."""
...
EPILOGUE_OVERLAP_KERNEL = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap"
)
REGISTER_PREFETCH_KERNEL = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata_register_prefetch"
)
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    overlap_out = torch.empty(
        shape.m, shape.n, device="cuda", dtype=torch.bfloat16
    )
    prefetch_out = torch.empty_like(overlap_out)
    deepgemm_out = torch.empty_like(overlap_out)
    cublas_out = torch.empty_like(overlap_out)

    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap(
        activation, packed, out=overlap_out
    )
    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_register_prefetch(
        activation, packed, out=prefetch_out
    )
    ...
    torch.testing.assert_close(prefetch_out, overlap_out, rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(overlap_out, deepgemm_out, rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(cublas_out, deepgemm_out, rtol=2e-2, atol=2e-2)

    overlap_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap(
            activation, packed, out=overlap_out
        ),
        EPILOGUE_OVERLAP_KERNEL,
        num_tests=num_tests,
    ...
    prefetch_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_register_prefetch(
            activation, packed, out=prefetch_out
        ),
        REGISTER_PREFETCH_KERNEL,
        num_tests=num_tests,
    ...
    print(
        ...
        f"{overlap_time * 1e6:11.2f} {prefetch_time * 1e6:12.2f} "
        f"{deepgemm_time * 1e6:10.2f} {cublas_time * 1e6:10.2f} "
        f"{safe_ratio(overlap_time, prefetch_time):16.3f}x "
        f"{safe_ratio(deepgemm_time, prefetch_time):10.3f}x"
    )
...
def main() -> None:
    ...
    print(
        "     M      N      K | overlap(us) prefetch(us) deepgemm(us) "
        "cublas(us) overlap/prefetch dg/prefetch"
    )
    for shape in shapes:
