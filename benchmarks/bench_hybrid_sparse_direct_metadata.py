"""Compare Stage 3 shared staging with direct lane-ready metadata loads."""
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    stage3_out = torch.empty(
        shape.m, shape.n, device="cuda", dtype=torch.bfloat16
    )
    direct_out = torch.empty_like(stage3_out)
    deepgemm_out = torch.empty_like(stage3_out)
    cublas_out = torch.empty_like(stage3_out)
    ...
    torch.testing.assert_close(direct_out, stage3_out, rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(stage3_out, deepgemm_out, rtol=2e-2, atol=2e-2)
    ...
    print(
        ...
        f"{stage3_time * 1e6:10.2f} {direct_time * 1e6:10.2f} "
        f"{deepgemm_time * 1e6:10.2f} {cublas_time * 1e6:10.2f} "
        f"{safe_ratio(stage3_time, direct_time):12.3f}x "
        f"{safe_ratio(deepgemm_time, direct_time):10.3f}x"
    )
...
def main() -> None:
    ...
    print(
        "     M      N      K | stage3(us) direct(us) deepgemm(us) "
        "cublas(us) stage3/direct dg/direct"
    )
    for shape in shapes:
