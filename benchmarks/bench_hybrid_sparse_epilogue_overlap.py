"""Compare direct metadata with producer overlap across the epilogue."""
...
DIRECT_METADATA_KERNEL = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_direct_metadata"
)
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    overlap_out = torch.empty_like(direct_out)
    deepgemm_out = torch.empty_like(direct_out)
    cublas_out = torch.empty_like(direct_out)

    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata(
        activation, packed, out=direct_out
    )
    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata_epilogue_overlap(
    ...
    torch.testing.assert_close(overlap_out, direct_out, rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(direct_out, deepgemm_out, rtol=2e-2, atol=2e-2)
    torch.testing.assert_close(cublas_out, deepgemm_out, rtol=2e-2, atol=2e-2)

    direct_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_direct_metadata(
            activation, packed, out=direct_out
        ),
        DIRECT_METADATA_KERNEL,
        num_tests=num_tests,
    ...
    print(
        ...
        f"{direct_time * 1e6:10.2f} {overlap_time * 1e6:11.2f} "
        f"{deepgemm_time * 1e6:10.2f} {cublas_time * 1e6:10.2f} "
        f"{safe_ratio(direct_time, overlap_time):14.3f}x "
        f"{safe_ratio(deepgemm_time, overlap_time):10.3f}x"
    )
...
def main() -> None:
    ...
    print(
        "     M      N      K | direct(us) overlap(us) deepgemm(us) "
        "cublas(us) direct/overlap dg/overlap"
    )
    for shape in shapes:
