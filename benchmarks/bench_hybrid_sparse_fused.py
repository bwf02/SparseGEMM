...
MERGE_K2_KERNEL_NAME = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_merge_k2"
)
...
def benchmark_shape(
    ...
    merge_k2_out = torch.empty_like(separate_out)
    ...
    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_merge_k2(
        activation, packed_weight, out=merge_k2_out
    )
    ...
    torch.testing.assert_close(merge_k2_out, lane_ready_out, rtol=2e-2, atol=2e-2)
    ...
    merge_k2_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_merge_k2(
            activation, packed_weight, out=merge_k2_out
        ),
        MERGE_K2_KERNEL_NAME,
        num_tests=num_tests,
    ...
    print(
        ...
        f"{lane_ready_time * 1e6:14.2f} {merge_k2_time * 1e6:12.2f} "
        ...
        f"{safe_divide(lane_ready_time, merge_k2_time):8.3f}x"
    )
...
def main() -> None:
    ...
    print(
        ...
        "persistent(us) lane-ready(us) merge-k2(us) deepgemm(us) cublas(us) "
        "direct/stsm dg/lane-ready cublas/lane-ready lane/merge"
    )
    for shape in shapes:
        benchmark_shape(
            shape,
