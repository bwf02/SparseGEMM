"""Compare Stage 3 with warpgroup register reallocation."""
...
REG_REALLOC_KERNEL = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_reg_realloc"
)
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    reg_realloc_out = torch.empty_like(stage3_out)
    ...
    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_reg_realloc(
        activation, packed, out=reg_realloc_out
    )
    ...
    torch.testing.assert_close(
        reg_realloc_out, stage3_out, rtol=2e-2, atol=2e-2
    )
    ...
    reg_realloc_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_reg_realloc(
            activation, packed, out=reg_realloc_out
        ),
        REG_REALLOC_KERNEL,
        num_tests=num_tests,
...
def main() -> None:
    ...
    print(
        "     M      N      K | stage3(us) reg-realloc(us) deepgemm(us) "
        "cublas(us) stage3/reg dg/reg"
    )
    for shape in shapes:
