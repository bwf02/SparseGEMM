...
STAGE5_KERNEL = (
    "hybrid_sparse_fused_wgmma_tma_stsm_persistent_lane_ready_stage5"
)
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    stage5_out = torch.empty_like(stage2_out)
    ...
    hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_stage5(
        activation, packed, out=stage5_out
    )
    ...
    torch.testing.assert_close(stage5_out, stage2_out, rtol=2e-2, atol=2e-2)
    ...
    stage5_time = bench_kineto(
        lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_stage5(
            activation, packed, out=stage5_out
        ),
        STAGE5_KERNEL,
        num_tests=num_tests,
    ...
    print(
        ...
        f"{stage4_time * 1e6:10.2f} {stage5_time * 1e6:10.2f} "
        f"{deepgemm_time * 1e6:10.2f} {cublas_time * 1e6:10.2f} "
        f"{stage2_time / stage5_time:10.3f}x "
        f"{deepgemm_time / stage5_time:10.3f}x"
    )
...
def main() -> None:
    ...
    print(
        "     M      N      K | stage2(us) stage3(us) stage4(us) stage5(us) "
        "deepgemm(us) cublas(us) stage2/stage5 dg/stage5"
    )
    for shape in shapes:
