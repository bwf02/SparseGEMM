"""Compare direct metadata with stage-local TMA metadata."""
...
def benchmark_shape(shape: Shape, num_tests: int, flush_l2: bool) -> None:
    ...
    tma_metadata_out = torch.empty_like(direct_out)
    ...
    torch.testing.assert_close(tma_metadata_out, direct_out, rtol=2e-2, atol=2e-2)
    ...
    print(
        ...
        f"{direct_time * 1e6:10.2f} {tma_metadata_time * 1e6:12.2f} "
        f"{deepgemm_time * 1e6:10.2f} {cublas_time * 1e6:10.2f} "
        f"{safe_ratio(direct_time, tma_metadata_time):14.3f}x "
        f"{safe_ratio(deepgemm_time, tma_metadata_time):10.3f}x"
    )
...
def main() -> None:
    ...
    print(
        "     M      N      K | direct(us) tma_meta(us) deepgemm(us) "
        "cublas(us) direct/tma_meta dg/tma_meta"
    )
    for shape in shapes:
