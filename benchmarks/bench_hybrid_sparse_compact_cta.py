"""Benchmark the 160-thread compact CTA against its 256-thread baseline."""
...
COMPACT_KERNEL = "hybrid_sparse_group_stage_output48x64_nm12_fastpath_compact"
...
def benchmark_shape(shape: Shape, repeats: int, num_tests: int) -> dict:
    ...
    compact_output = torch.empty_like(baseline_output)
    ...
    compact_call = lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output48x64_nm12_fastpath_compact(
        activation, packed, out=compact_output
    )
    ...
    torch.testing.assert_close(
        compact_output, deepgemm_output, rtol=2e-2, atol=2e-2
    )

    timings = {"baseline": [], "compact": [], "deepgemm": []}
    measurements = (
        ("baseline", baseline_call, BASELINE_KERNEL),
        ("compact", compact_call, COMPACT_KERNEL),
    ...
    return {
        ...
        **{
            f"{name}_median_us": statistics.median(values)
            for name, values in timings.items()
        },
        "speedup_over_deepgemm": statistics.median(
            deepgemm / compact
            for compact, deepgemm in zip(
                timings["compact"], timings["deepgemm"]
            )
        ),
    }
...
def main() -> None:
    ...
    results = []
    print("     M      N      K | baseline compact DeepGEMM speedup")
    for shape in qwen_moe_shapes(args.m):
        ...
        print(
            ...
            f"{result['compact_median_us']:7.2f} "
