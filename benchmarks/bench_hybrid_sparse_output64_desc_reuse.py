...
def benchmark_shape(shape: Shape, repeats: int, num_tests: int) -> dict:
    ...
    uniform_desc_out = torch.empty_like(generic_out)
    constexpr_desc_out = torch.empty_like(generic_out)
    constexpr_desc_branch_group_out = torch.empty_like(generic_out)
    constexpr_desc_branch_group_full_grid_out = torch.empty_like(generic_out)
    ...
    uniform_desc_call = lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_stage7_fused_mma_group_uniform_desc(
        activation, packed, out=uniform_desc_out
    )
    constexpr_desc_call = lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_stage7_fused_mma_group_constexpr_desc(
        activation, packed, out=constexpr_desc_out
    )
    constexpr_desc_branch_group_call = lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_stage7_fused_mma_group_constexpr_desc_branch_group(
        activation, packed, out=constexpr_desc_branch_group_out
    )
    constexpr_desc_branch_group_full_grid_call = lambda: hybrid_block_sparse_gemm_wgmma_tma_fused_stsm_persistent_lane_ready_group_stage_output64x64_nm12_fastpath_desc_reuse_fixed_shape_stage7_fused_mma_group_constexpr_desc_branch_group_full_grid(
        activation, packed, out=constexpr_desc_branch_group_full_grid_out
    )
    ...
    for function in (
        ...
        uniform_desc_call,
        constexpr_desc_call,
        constexpr_desc_branch_group_call,
        constexpr_desc_branch_group_full_grid_call,
    ...
    for actual in (
        ...
        uniform_desc_out,
        constexpr_desc_out,
        constexpr_desc_branch_group_out,
        constexpr_desc_branch_group_full_grid_out,
    ...
    measurements = (
        ...
        (
            "constexpr_desc_branch_group",
            constexpr_desc_branch_group_call,
            "hybrid_sparse_group_stage_output64x64",
        ),
        (
            "constexpr_desc_branch_group_full_grid",
            constexpr_desc_branch_group_full_grid_call,
            "hybrid_sparse_group_stage_output64x64",
        ),
        (
            "output32_unroll_k",
            output32_unroll_k_call,
            "hybrid_sparse_group_stage_output32x64_nm12_fastpath_desc_reuse_fixed_shape_stage7_unroll_k",
        ),
        (
            "async_group3",
            async_group3_call,
    ...
    return {
        ...
        "unroll_k_over_stage7": medians["stage7"] / medians["unroll_k"],
        "unroll_k_speedup_over_deepgemm": medians["deepgemm"] / medians["unroll_k"],
...
def main() -> None:
    ...
    print(
        "     M      N      K | generic fastpath reuse fixed stage7 unrollK "
        "async3 DeepGEMM unrollK/stage7 DG/unrollK"
    )
    results = []
    for m in args.m:
        ...
        print(
            ...
            f"{medians['unroll_k']:7.2f} "
            f"{medians['async_group3']:6.2f} "
            f"{medians['deepgemm']:8.2f} "
            f"{result['unroll_k_over_stage7']:15.3f}x "
            f"{result['unroll_k_speedup_over_deepgemm']:10.3f}x",
            flush=True,
        )
